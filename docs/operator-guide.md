...
```
mnemos-server  # Go single binary
...
```bash
mnemos serve --config /etc/mnemos/config.toml
# or
docker run -d mnemos/mnemos:latest
```
...
```
mnemos-server  + Postgres (with pgvector)
...
mnemos schema 版本号每条 memory 携带（`schema_version` 字段）。升级路径：
1. 先备份
2. 阅读 release notes 含 BREAKING-CHANGES 章节
3. 跑 `mnemos migrate --dry-run` 验证
4. 跑 `mnemos migrate` 应用
...
- `mnemos_query_latency_p50_ms` / `_p99_ms` — hot-path 关键
- `mnemos_storage_bytes` — 容量增长
- `mnemos_tenant_count` / `_user_count`
- `mnemos_write_qps` / `_read_qps`
- `mnemos_decay_jobs_completed_total`
- `mnemos_contradiction_detected_total`
- `mnemos_export_requests_total`
