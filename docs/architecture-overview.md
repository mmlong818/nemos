...
旗舰形态是一款**多人格 AI 陪伴 App**（微信式聊天，几个会跨时间记得你的 AI 角色）；**底层是一套可独立抽出的记忆系统**——陪伴是外壳，记忆是内核。本文档讲的是这套记忆内核：一个 source-available 的 personal memory infrastructure，让陪伴 App 里的角色、以及任何接入的 AI 应用（Claude Code、Cursor、ChatGPT、自定义 agent）能跨 session / 跨工具持有"关于用户的同一个我"，同时用户对自己的 AI 记忆保有审计、纠正、删除、迁移的主权。

## 记忆内核不是什么

- ❌ 不是 vector DB（虽然内部用了 pgvector / hnswlib）
- ❌ 不是 RAG 框架
- ❌ 记忆内核本身不是用户直接打开的界面（用户面向的是上层陪伴 App；内核也可被其他 AI app 当底层接入）
...
12. source-available（禁止商用）PolyForm Noncommercial 1.0.0
