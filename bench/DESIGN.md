...
- **可追溯**:固定数据、记录模型 ID、温度、prompt 版本和结果哈希；托管模型仍可能引入运行间差异。
- **消融友好**:同一套数据喂给完整配置与关闭单项机制的配置，用差值评估该机制在当前实验设置下的贡献。
...
**指标**:
- **TL (Trivia Leakage)**:检索结果出现应被冷却琐碎信息的比例。↓
- **IFR (Important-Fact Retention)**: 重要事实保留率(守卫,必须≈1)。↓丢失即判失败。
- 假设:Nemos(FSRS 衰减)TL 低于 no-decay 消融;IFR 不下降。
...
- 论文主表实际使用 BUC、ASP、FOR 各 **50 条任务**；ASP 共 250 个 probe，FOR 共 146 个 probe。
- LongMemEval 交叉锚点使用知识更新切片的 **30 个问题**。
- 生成器和 judge 使用 `gpt-4o`，embedding 使用 `text-embedding-3-small`；这是冻结实验配置，不代表当前模型推荐。
- 数据、生成器、judge prompt 与原始结果均在仓库内；提交版本、运行日期和 SHA-256 见结果 manifest。
- 托管模型、依赖版本和上游 LongMemEval 数据都可能漂移，因此“同配置重跑”不保证逐位一致。

## 6. 对照组(基线)

| 变体 | 配置 | 验证的机制 |
|---|---|---|
| `nemos-v2-semantic` | 语义失效、命名空间隔离、衰减开启 | 论文主配置 |
...
- **judge 偏差**:judge 用与被测无关的判定任务和固定低温降低方差；仓库未包含可核验的人工抽检记录，因此不把人工抽检写成已完成事实。
- **生成器泄漏**:渲染会话的 LLM 与 judge 的 LLM 不共享真值脚本以外的信息。
- **规模**:每项 50 条任务和 30 题交叉锚点仍属小样本，结果只支持当前合成数据与冻结配置，不外推为生产环境上限。
