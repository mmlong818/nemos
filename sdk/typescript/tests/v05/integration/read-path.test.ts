...
// 验证 Mnemos.search 四级激活 rerank + getRelevantContext 前瞻通道；
// 关键回归：features 默认关时行为等价 v0.4。

import { test } from "node:test";
import assert from "node:assert/strict";
import { Mnemos, type RouteResult } from "../../../src/index.js";
import { makeMockLLMConfig } from "../../helpers.js";

const TENANT = "default";

// 把三条 memory（2 econ + 1 med）写入并归属领域。
async function seed(mem: Mnemos, user: string): Promise<{ econ1: string; econ2: string; med1: string }> {
