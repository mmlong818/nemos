...
test("model adapters forward exact efforts on Responses and Chat Completions without budget changes", async () => {
  ...
  try {
    for (const id of ["gpt-6-astra", "gpt-5.6-terra", "gpt-5.6-luna"]) {
      for (const effort of [...supportedReasoningEfforts(connection, id), undefined]) {
        ...
        assert.equal(id === "gpt-6-astra" ? body.reasoning?.effort : body.reasoning_effort, effort);
        assert.equal(id === "gpt-6-astra" ? body.max_output_tokens : body.max_completion_tokens, 80);
