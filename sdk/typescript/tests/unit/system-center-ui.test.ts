...
test("设置中心仅保留模型、连接与本机数据", () => {
  ...
  assert.match(server, /系统代理不可用；请求已被阻止且没有回退直连/);
  assert.match(server, /API Key 无效或未被服务接受/);
...
test("模型设置分开管理官方短名单、账号原始目录和显式能力检查", () => {
  const html = readWeb("settings.html");
  const script = readWeb(join("assets", "settings-center.js"));
  const server = readServerRouteSurface();
  ...
  assert.match(html, /可能产生(?:极少量)?费用/);
  assert.match(script, /renderModelCatalog/);
  assert.match(script, /账号原始目录/);
  assert.match(script, /官方推荐/);
  assert.match(script, /用途设置/);
  assert.match(script, /看图理解/);
  assert.match(script, /语音输入/);
  assert.match(script, /朗读回复/);
  assert.match(script, /默认思考强度/);
  assert.match(script, /data-reasoning-scope/);
  assert.match(script, /captureModelUiState/);
  assert.match(script, /restoreModelUiState/);
  assert.match(script, /syncPolicyControlsFromServer/);
  assert.match(script, /cache: "no-store"/);
  assert.match(script, /applicationOverridesOpen/);
  ...
  assert.match(html, /settings-center\.js\?v=0\.7\.6-modelux4/);
  const ttsPlayer = html.indexOf("tts-preview-player.js?v=0.7.6-media1");
  const settingsCenter = html.indexOf("settings-center.js?v=0.7.6-modelux4");
  ...
  assert.doesNotMatch(script, /finally \{ (?:reasoningSelect|select)\.disabled = false; \}/);
  assert.match(script, /\/api\/llm-chat-policy/);
  assert.match(script, /center\.scenes/);
  assert.match(script, /scene\.executionState === "wired"/);
  assert.match(script, /scene\.modelOverride/);
  assert.match(script, /scene\.reasoningOverride/);
  assert.doesNotMatch(script, /const sceneLabels\s*=/);
  assert.doesNotMatch(script, /const sceneCapabilities\s*=/);
  assert.match(server, /MODEL_APPLICATION_SCENES/);
  assert.doesNotMatch(script, /reasoning:\s*"深度思考"/);
  assert.match(script, /support\.reason/);
  assert.doesNotMatch(script, /\["image_generation", "video_generation"\]\.includes\(capability\)/);
  assert.match(server, /fetchCompanionModelCatalog/);
  assert.match(script, /\/api\/llm-model\/register-enable/);
  assert.doesNotMatch(script, /\/api\/llm-model\/favorite|data-model-favorite|收藏|加入常用|移出常用/);
  assert.match(script, /\/api\/llm-model\/catalog/);
  assert.match(script, /\/api\/llm-model\/check/);
  assert.doesNotMatch(script, /官方资料：|资料更新于|· 证据 \$\{/);
  assert.doesNotMatch(readWeb("settings.html"), /登记目录外 ID/);
  assert.match(script, /账号目录没有返回这个型号；系统会先加入当前连接/);
