...
test("v0.6 companion 群聊: 在场扩散 + 1-on-1 仍分隔 + 非成员看不到", async () => {
  const dir = mkdtempSync(join(tmpdir(), "nemos-comp-group-"));
  try {
    const { mem, engine } = setup(join(dir, "t.db"));
    engine.createGroup("g1", ["yeque", "xiaohang"]); // 沐 不在群

    // 只对夜雀 1-on-1 说 Max（私密）
    await engine.send("alice", "yeque", "我养了一只狗叫 Max");
    // 群里说 Tokyo（在场扩散给群成员）
    const replies = await engine.sendToGroup("alice", "g1", "我下周要搬去 Tokyo");
    assert.equal(replies.length, 2, "群里每个成员各回一条");
    assert.deepEqual(replies.map((r) => r.personaId).sort(), ["xiaohang", "yeque"]);

    // 在场扩散：群成员都记得 Tokyo
    assert.ok((await engine.recall("alice", "yeque", "Tokyo")).userFacts.includes("Tokyo"), "夜雀记得群里的 Tokyo");
    assert.ok((await engine.recall("alice", "xiaohang", "Tokyo")).userFacts.includes("Tokyo"), "小航也记得（在场扩散）");

    // 1-on-1 仍分隔：小航看不到只对夜雀说过的 Max
    assert.ok(!(await engine.recall("alice", "xiaohang", "Max")).userFacts.includes("Max"), "小航看不到夜雀的 1-on-1 私事");
    // 非成员看不到群内容
    assert.ok(!(await engine.recall("alice", "mu", "Tokyo")).userFacts.includes("Tokyo"), "沐不在群 → 看不到 Tokyo");
...
test("v0.6 companion 语音条: 走 voice-transcript profile，内容照样被记住", async () => {
  const dir = mkdtempSync(join(tmpdir(), "nemos-comp-voice-"));
  try {
    const { mem, engine } = setup(join(dir, "t.db"));
    const r = await engine.send("alice", "yeque", "我最近爱上喝 latte", { voice: { durationSec: 8 } });
    assert.equal(typeof r.reply, "string", "语音条走通");
    const ctx = await engine.recall("alice", "yeque", "latte");
    assert.ok(ctx.userFacts.includes("latte"), "语音条内容照样进记忆、可召回");
