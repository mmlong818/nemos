...
(() => {
  ...
  async function loadPlatform() { try { const [platform, extensions] = await Promise.all([api("/api/platform/readiness"), api("/api/agent/extensions")]); renderTools(platform.development); renderConnections(platform.connectors); renderExtensions(extensions.extensions); } catch (error) { $("#connectionStatus").className = "status error"; $("#connectionStatus").textContent = error.message; } }
