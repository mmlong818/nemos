  ...
  return {
    ...
    def: {
      ...
      function: {
        name: "web_search",
        description:
          "联网搜实时硬事实（行情股价、汇率、天气、新闻赛事、某产品/公司的最新状况）。" +
          "你是朋友身份的聊天对象，不是搜索助手：情绪倾诉、日常闲聊、给安慰或建议、你已知的常识，一律不要调用，直接用自己的话回应。",
        parameters: {
          type: "object",
...
const TOOL_POLICY =
  "\n\n【联网能力】你可以联网搜索（web_search），但默认不用。仅当问题确实依赖实时 / 最新信息时才调用——" +
  "如行情股价、汇率、天气、新闻赛事、某公司或产品的最新状况（是否上市 / 市值 / 融资）、" +
  "或带「最新 / 现在 / 今天 / 近期 / 多少钱」且你不确定的事实；这类别凭旧记忆下结论。" +
  "日常聊天、情绪倾诉（如「好累」「有点烦」）、安慰、给建议、你已知道的常识，绝不调用——直接用你自己的话回应。" +
  "就算真的搜了，也只把有用的一两句揉进自己的话里，绝不要写「根据我查到的资料」、不要列 1.2.3. 条目——你是朋友，不是在交报告。";

// 工具按需挂载：弱模型不听"别搜"的话，所以日常/情绪聊天根本不把 web_search 给它。
// 只在用户这句话确实像在问实时硬事实时才挂工具。只看"对方："（用户）说的话，
// 不被角色自己上一条回复（可能恰好提了天气）带偏。
const WEB_CUES =
  /(天气|气温|下雨|台风|降温|暴雨|股价|股票|大盘|指数|基金|涨跌|汇率|美元|日元|欧元|英镑|比特币|油价|金价|房价|多少钱|报价|新闻|头条|发生了什么|出什么事|比分|赛事|比赛|夺冠|利率|融资|市值|上市|IPO|财报|航班|车次|路况|限号|确诊|上映|票房|排名|榜单)/i;
...
function makeZhipuChat(apiKey: string, defaultModel: string, tools: Tool[] = []): ChatFn {
  ...
  return async (system: string, user: string, model?: string, maxTokens?: number): Promise<string> => {
    ...
    const useTools = toolDefs.length > 0 && mightNeedWeb(user);
    const sys = useTools
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (useTools) body.tools = toolDefs;
      const resp = await fetch(ZHIPU_CHAT_ENDPOINT, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
...
function makeZhipuChatStream(apiKey: string, defaultModel: string, tools: Tool[] = []): ChatStreamFn {
  ...
  return async (system, user, cb, model, maxTokens) => {
    ...
    const useTools = toolDefs.length > 0 && mightNeedWeb(user);
    const messages: Array<Record<string, unknown>> = [
      { role: "system", content: useTools ? `${system}${TOOL_POLICY}\n（现在是 ${now}（北京时间），引用搜索结果注意时效。）` : system },
      { role: "user", content: user },
    ];
    cb.onStatus("工作中");
    let full = "";
