...
技能库（`/bots?view=bots`）内含 5 个随应用发布的原生适配规则模板：项目推进、会前准备、文稿润色、方案压力测试和 Bot 设计。可查看完整规则与能力边界，修改名称、规则和启用状态；保存规则不调用模型，任务才调用已配置的模型。这些模板是随应用发布的本地目录，不自动下载第三方脚本或同步 Grok 账号。深度研究与核验会检索、分级并复核来源；材料梳理只处理本次已经提供的材料，两者互补而不互相替代。详见[市场适配与验证](../../../../docs/bot-market-adoption-2026-09-06.md)。
