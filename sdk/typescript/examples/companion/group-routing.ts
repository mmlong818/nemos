...
const EXPERT_TOPIC_RULES: Array<{ ids: string[]; pattern: RegExp }> = [
  ...
  { ids: ["security_compliance", "system_architecture"], pattern: /安全|隐私|权限|密钥|令牌|凭证|泄露|合规|审计/i },
  { ids: ["quality_testing", "lean_engineering"], pattern: /测试|质量|故障|错误|缺陷|验收|稳定|可靠/i },
  { ids: ["research_verification", "industry_analysis"], pattern: /研究|检索|来源|证据|核验|论文|仓库|实际实现|真假/i },
  { ids: ["industry_analysis", "data_analysis"], pattern: /市场|行业|产业|竞品|数据|表格|Excel|指标|统计|图表/i },
  { ids: ["pricing_finance", "decision_analysis"], pattern: /定价|成本|收入|财务|预算|利润/i },
  { ids: ["document_editor", "brand_strategy"], pattern: /文档|Word|报告|文章|润色|改写|校对|正式稿/i },
  { ids: ["presentation_design", "visual_analysis"], pattern: /PPT|演示|幻灯片|路演|放映|构图|图像|提示词/i },
  { ids: ["project_coordination", "workflow_automation"], pattern: /会议|纪要|进度|项目|行动项|负责人|依赖|阻塞|自动化|工作流/i },
  { ids: ["sales_growth", "contract_risk"], pattern: /销售|增长|获客|转化|渠道|客户|合同|协议|合作|条款|谈判/i },
