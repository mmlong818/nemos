import { renderNavigationIcon, type NavigationIconName } from "./navigation-icons.js";

/**
 * Presentation-only shell. Existing pages own their actions and data.
 *
 * 这张表只用于核对"每条产品路由都在导航里出现过"，不参与渲染，所以不带图标字段：
 * 图标的唯一来源是下面 primary/tools/legacy 三组里的 NavigationIconName。
 */
export const WORKBENCH_LINKS = [
  ...
  { href: "/skills", label: "技能库", group: "tools" },
...
type NavigationEntry = { href: string; label: string; icon: NavigationIconName; id?: string; productKey?: string };
...
export function renderWorkbenchNavigation(path: string): string {
  ...
  const link = (item: NavigationEntry) => {
    ...
    return `<a class="wb-link"${item.id ? ` id="${item.id}"` : ""} href="${item.href}" data-wb-path="${wbPath}"${current}${item.productKey ? ` data-product-key="${item.productKey}"` : ""}><span aria-hidden="true">${renderNavigationIcon(item.icon)}</span>${item.label}</a>`;
  };
  const primary: NavigationEntry[] = [
    ...
    { href: "/memory", label: "记忆", icon: "memory", productKey: "memory" },
  ];
  const tools: NavigationEntry[] = [
    { href: "/skills", label: "技能库", icon: "skills", id: "railSkills", productKey: "bots" },
    { href: "/capabilities", label: "工具与连接", icon: "plug", id: "railCap", productKey: "tools" },
  ];
