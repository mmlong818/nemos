...
function workbenchScript(kind: NativeCapabilityId): string {
  const common = 'document.querySelectorAll("[data-copy]").forEach(b=>b.onclick=async()=>{await navigator.clipboard.writeText(b.dataset.copy||"");const old=b.textContent;b.textContent="已复制";setTimeout(()=>b.textContent=old,1200);});const key="clownfish-workbench-"+document.title;let saved={};try{saved=JSON.parse(localStorage.getItem(key)||"{}")}catch{};const noteFields=[...document.querySelectorAll("#workNotes,#workbenchNotes")];const status=document.getElementById("workbenchStatus");const checks=[...document.querySelectorAll("[data-check]")];const versions=document.getElementById("workbenchVersions");const diff=document.getElementById("workbenchDiff");function capture(){const notes={};noteFields.forEach(x=>notes[x.id]=x.value);const checked={};checks.forEach(x=>checked[x.dataset.check]=x.checked);return{notes,checked,status:status?status.value:"draft"}}function persist(){saved.current=capture();localStorage.setItem(key,JSON.stringify(saved))}function drawVersions(){if(!versions)return;const rows=saved.versions||[];versions.innerHTML=rows.length?rows.map(v=>"<option value="+JSON.stringify(v.id)+">"+new Date(v.at).toLocaleString()+" · "+v.status+"</option>").join(""):"<option value=\\"\\">尚未保存版本</option>"}const current=saved.current||{};noteFields.forEach(x=>{x.value=current.notes&&current.notes[x.id]||"";x.addEventListener("input",persist)});checks.forEach(x=>{x.checked=!!(current.checked&&current.checked[x.dataset.check]);x.addEventListener("change",persist)});if(status){status.value=current.status||"draft";status.addEventListener("change",persist)};document.getElementById("workbenchSaveVersion")?.addEventListener("click",()=>{const value=capture();saved.versions=[{id:crypto.randomUUID(),at:Date.now(),...value},...(saved.versions||[])].slice(0,12);saved.current=value;localStorage.setItem(key,JSON.stringify(saved));drawVersions();if(diff)diff.textContent="版本已保存。"});document.getElementById("workbenchRestoreVersion")?.addEventListener("click",()=>{const value=(saved.versions||[]).find(v=>v.id===versions.value);if(!value)return;const before=capture();noteFields.forEach(x=>x.value=value.notes&&value.notes[x.id]||"");checks.forEach(x=>x.checked=!!(value.checked&&value.checked[x.dataset.check]));if(status)status.value=value.status||"draft";persist();if(diff){const noteChanged=JSON.stringify(before.notes)!==JSON.stringify(value.notes);const checkChanged=JSON.stringify(before.checked)!==JSON.stringify(value.checked);diff.textContent="已恢复 · "+(noteChanged?"备注有变化":"备注相同")+" · "+(checkChanged?"勾选有变化":"勾选相同")}});drawVersions();';
