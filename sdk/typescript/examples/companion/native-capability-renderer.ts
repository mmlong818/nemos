...
function renderProduct(payload: NativeCapabilityPayload): string {
  ...
  const tokens = record(data.designTokens);
  const accent = hexColor(text(tokens.accent), "#b85c38");
  const background = hexColor(text(tokens.background), "#f2eee5");
  const surface = hexColor(text(tokens.surface), "#fffdf8");
  const foreground = hexColor(text(tokens.text), "#292823");
  return `${heroBlock("用户任务", text(data.job), [text(data.user), `${screens.length} 个关键界面`, "含状态与验收"])}
    ...
    <section class="panel design-workbench" id="designWorkbench" style="--design-accent:${accent};--design-background:${background};--design-surface:${surface};--design-text:${foreground}">
      <div class="section-head"><div><p class="kicker">可编辑设计画布</p><h2>关键界面</h2></div><div class="viewport-switch" aria-label="预览尺寸"><button type="button" data-viewport="desktop" class="is-active">桌面</button><button type="button" data-viewport="tablet">平板</button><button type="button" data-viewport="mobile">手机</button></div></div>
      <div class="design-token-bar" aria-label="设计令牌">
        <label>强调色<input id="designTokenAccent" data-workbench-value data-design-token="--design-accent" type="color" value="${accent}"></label>
        <label>背景<input id="designTokenBackground" data-workbench-value data-design-token="--design-background" type="color" value="${background}"></label>
        <label>卡片<input id="designTokenSurface" data-workbench-value data-design-token="--design-surface" type="color" value="${surface}"></label>
        <label>文字<input id="designTokenText" data-workbench-value data-design-token="--design-text" type="color" value="${foreground}"></label>
      </div>
      <div class="screen-tabs">${screens.map((screen, index) => `<button type="button" data-screen-tab="${index}" class="${index === 0 ? "is-active" : ""}">${escapeHtml(text(screen.name))}</button>`).join("")}</div>
      ${screens.map((screen, index) => `<article class="design-screen ${index === 0 ? "is-active" : ""}" data-screen="${index}">
        <form class="design-inspector" aria-label="${escapeAttribute(text(screen.name))}界面内容" onsubmit="return false">
          <label>界面名称<input id="design-${index}-name" data-workbench-value data-design-field="name" value="${escapeAttribute(text(screen.name))}"></label>
          <label>界面目的<textarea id="design-${index}-purpose" data-workbench-value data-design-field="purpose">${escapeHtml(text(screen.purpose))}</textarea></label>
          <label>主要操作<input id="design-${index}-action" data-workbench-value data-design-field="action" value="${escapeAttribute(text(screen.primaryAction) || "主要操作")}"></label>
          <label>内容区（每行一个）<textarea id="design-${index}-sections" data-workbench-value data-design-field="sections">${escapeHtml(strings(screen.sections).join("\n"))}</textarea></label>
          <label>状态（每行一个）<textarea id="design-${index}-states" data-workbench-value data-design-field="states">${escapeHtml(strings(screen.states).join("\n"))}</textarea></label>
        </form>
        <div class="screen-preview is-active" data-design-preview><div class="screen-top"><span></span><span></span><span></span></div><div class="screen-body"><p class="kicker" data-design-output="purpose">${escapeHtml(text(screen.purpose))}</p><h3 data-design-output="name">${escapeHtml(text(screen.name))}</h3><button type="button" data-design-output="action">${escapeHtml(text(screen.primaryAction) || "主要操作")}</button><div class="mock-sections" data-design-output="sections">${strings(screen.sections).map((item) => `<div>${escapeHtml(item)}</div>`).join("")}</div><p class="state-line" data-design-output="states">状态：${strings(screen.states).map(escapeHtml).join(" / ")}</p></div></div>
      </article>`).join("")}
      <p class="save-hint">文案、页面状态、配色和预览尺寸可以直接调整；内容会随工作台版本保存在本机。</p>
    </section>
...
function hexColor(value: string, fallback: string): string {
  ...
  if (/^#[0-9a-f]{3}$/i.test(source)) return `#${source.slice(1).split("").map((part) => part + part).join("")}`.toLowerCase();
  return fallback;
}
...
function workbenchScript(kind: NativeCapabilityId): string {
  ...
  const product = 'const design=document.getElementById("designWorkbench"),tabs=[...document.querySelectorAll("[data-screen-tab]")],screens=[...document.querySelectorAll("[data-screen]")];tabs.forEach((tab,i)=>tab.onclick=()=>{tabs.forEach((x,n)=>x.classList.toggle("is-active",n===i));screens.forEach((x,n)=>x.classList.toggle("is-active",n===i));});document.querySelectorAll("[data-viewport]").forEach(button=>button.onclick=()=>{document.querySelectorAll("[data-viewport]").forEach(x=>x.classList.toggle("is-active",x===button));design.dataset.viewport=button.dataset.viewport;});document.querySelectorAll("[data-design-token]").forEach(input=>input.addEventListener("input",()=>design.style.setProperty(input.dataset.designToken,input.value)));const lines=value=>value.split(/\\n/).map(x=>x.trim()).filter(Boolean);screens.forEach(screen=>{const preview=screen.querySelector("[data-design-preview]");screen.querySelectorAll("[data-design-field]").forEach(input=>{const update=()=>{const field=input.dataset.designField,target=preview.querySelector(`[data-design-output="${field}"]`);if(!target)return;if(field==="sections"){target.replaceChildren(...lines(input.value).map(value=>{const node=document.createElement("div");node.textContent=value;return node;}));}else if(field==="states")target.textContent="状态："+lines(input.value).join(" / ");else target.textContent=input.value|| (field==="action"?"主要操作":"未命名界面");if(field==="name"){const index=screens.indexOf(screen);if(tabs[index])tabs[index].textContent=input.value||"未命名界面";}};input.addEventListener("input",update);update();});});';
