...
function renderWorkbench(payload: NativeCapabilityPayload, metadata?: NativeArtifactMetadata): string {
  ...
  const content = renderWorkbenchContent(payload, metadata);
...
function renderPresentation(payload: NativeCapabilityPayload): string {
  ...
  const content = `<main class="deck" aria-label="演示文稿">${slides.map((slide, index) => {
...
function renderResearch(payload: NativeCapabilityPayload): string {
  ...
  return `${heroBlock("研究问题", text(data.question), ["分阶段研究", "来源分级", "结论核验"])}
    ...
    ${textBlock("结论", text(data.conclusion))}${listBlock("限制与待确认", strings(data.limitations))}${listBlock("下一步", strings(data.nextSteps))}`;
}
...
function renderMarket(payload: NativeCapabilityPayload): string {
  ...
  return `${heroBlock("机会假设", text(data.thesis), [text(data.targetUser), "三情景模拟", "含失效条件"])}${textBlock("需要解决的问题", text(data.problem))}
    ...
    ${listBlock("当前替代方案", strings(data.alternatives))}${listBlock("失效条件", strings(data.invalidation))}<section class="panel"><h2>验证实验</h2>${records(data.experiments).map((item) => `<article class="experiment"><h3>${escapeHtml(text(item.name))}</h3><p>成本 ${escapeHtml(text(item.cost))} · 周期 ${escapeHtml(text(item.duration))}</p><small>成功信号：${escapeHtml(text(item.successSignal))}</small></article>`).join("")}</section>${listBlock("主要风险", strings(data.risks))}`;
}
...
function renderAbility(payload: NativeCapabilityPayload, metadata?: NativeArtifactMetadata): string {
  ...
  return `${heroBlock(qualification.shouldBuild === true ? "适合沉淀为能力" : "暂不建议沉淀", text(qualification.reason), [text(spec.name), strings(spec.triggerExamples).length + " 个触发例", strings(data.testCases).length + " 个测试"])}
    ${installed ? `<section class="notice success"><strong>已加入 Nemos 能力库</strong><span>能力编号 ${escapeHtml(installed)}</span></section>` : `<section class="notice"><strong>尚未加入能力库</strong><span>资格检查未通过或未完成安装。</span></section>`}
...
function pageShell(title: string, summary: string, content: string, script: string, payload: NativeCapabilityPayload, mode = "workbench"): string {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>${artifactCss()}</style></head><body class="${mode}"><header class="artifact-header"><div><span class="brand-mark">N</span><span>Nemos 能力结果</span></div><button type="button" id="printButton">打印 / 导出 PDF</button></header><div class="artifact-wrap"><header class="title-block"><p>已完成</p><h1>${escapeHtml(title)}</h1><span>${escapeHtml(summary)}</span></header>${content}<footer>由 Nemos 在本机生成 · 具体要求优先于记忆偏好</footer></div><script type="application/json" id="artifactData">${safeJson}</script><script>document.getElementById('printButton').onclick=()=>print();${script}</script></body></html>`;
}
...
function workbenchScript(kind: NativeCapabilityId): string {
  ...
  if (kind === "thinking-workbench") return common + `const key='nemos-workbench-'+document.title;const saved=JSON.parse(localStorage.getItem(key)||'{}');document.querySelectorAll('[data-check]').forEach(x=>{x.checked=!!saved[x.dataset.check];x.onchange=save;});const notes=document.getElementById('workNotes');notes.value=saved.notes||'';notes.oninput=save;function save(){const data={notes:notes.value};document.querySelectorAll('[data-check]').forEach(x=>data[x.dataset.check]=x.checked);localStorage.setItem(key,JSON.stringify(data));}`;
  if (kind === "product-design") return common + `const tabs=[...document.querySelectorAll('[data-screen-tab]')];const screens=[...document.querySelectorAll('[data-screen]')];tabs.forEach((tab,i)=>tab.onclick=()=>{tabs.forEach((x,n)=>x.classList.toggle('is-active',n===i));screens.forEach((x,n)=>x.classList.toggle('is-active',n===i));});`;
  if (kind === "market-opportunity") return common + `const inputs=[...document.querySelectorAll('.weights input')];function update(){const d=+document.getElementById('demandWeight').value/100,e=+document.getElementById('executionWeight').value/100,c=+document.getElementById('competitionWeight').value/100;inputs.forEach(x=>x.nextElementSibling.textContent=x.value+'%');document.querySelectorAll('[data-scenario]').forEach(x=>{const score=Math.max(0,Math.min(100,Math.round(+x.dataset.demand*d + +x.dataset.execution*e + (100- +x.dataset.competition)*c)));x.querySelector('.score-ring strong').textContent=score;});}inputs.forEach(x=>x.oninput=update);update();`;
  return common;
}
