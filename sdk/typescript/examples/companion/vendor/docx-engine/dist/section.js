...
exports.sectionSettingsFromXml = sectionSettingsFromXml;
exports.readSectionSettings = readSectionSettings;
exports.applyPageNumType = applyPageNumType;
exports.readSections = readSections;
exports.applySectionSettings = applySectionSettings;
exports.applySectionStartType = applySectionStartType;
exports.readPageColor = readPageColor;
...
function intAttr(tag, name, fallback) {
...
function sectionSettingsFromXml(xml) {
    ...
    if (docGridTag) {
        ...
        const gridType = (typeMatch?.[1] ?? 'default');
    ...
    return {
        pageWidth: intAttr(pgSz, 'w:w', exports.DEFAULT_SECTION.pageWidth),
        pageHeight: intAttr(pgSz, 'w:h', exports.DEFAULT_SECTION.pageHeight),
        orientation: pgSz.includes('w:orient="landscape"') ? 'landscape' : 'portrait',
        marginTop: intAttr(pgMar, 'w:top', exports.DEFAULT_SECTION.marginTop),
        marginRight: intAttr(pgMar, 'w:right', exports.DEFAULT_SECTION.marginRight),
        marginBottom: intAttr(pgMar, 'w:bottom', exports.DEFAULT_SECTION.marginBottom),
        marginLeft: intAttr(pgMar, 'w:left', exports.DEFAULT_SECTION.marginLeft),
...
function readSectionSettings(parsed) {
...
function sectionFromSectPr(sectPrXml, firstBlockIndex, lastBlockIndex) {
    ...
    return {
        ...
        startType: type === 'nextColumn' ? 'continuous' : (type ?? 'nextPage'),
        firstBlockIndex,
        lastBlockIndex,
        sectPrXml,
...
function applyPageNumType(sectPrXml, fmt, start) {
    ...
    if (fmt === undefined && start === undefined)
        return xml;
    const tag = `<w:pgNumType${fmt !== undefined ? ` w:fmt="${fmt}"` : ''}${start !== undefined ? ` w:start="${start}"` : ''}/>`;
    if (/<w:cols[\s/>]/.test(xml))
        xml = xml.replace(/(<w:cols[\s/>])/, `${tag}$1`);
    else if (/<w:docGrid/.test(xml))
        xml = xml.replace(/(<w:docGrid)/, `${tag}$1`);
    else
        xml = xml.replace(/<\/w:sectPr>/, `${tag}</w:sectPr>`);
    return xml;
}
/**
 ...
 */
function readSections(parsed) {
    const sections = [];
    let first = 0;
    for (const block of parsed.blocks) {
        const xml = block.originalXml ?? '';
        if (block.docxIndex == null || !xml.includes('<w:sectPr'))
            continue;
...
function applySectionSettings(sectPrXml, settings) {
    ...
    const replaceMarAttr = (tag, name, value) => {
    ...
    if (marMatch) {
        ...
        const pgMar = `<w:pgMar w:top="${settings.marginTop}" w:right="${settings.marginRight}"` +
    ...
    if (settings.pageBorder) {
        ...
        const pgBorders = '<w:pgBorders w:offsetFrom="page">' +
    ...
    if (colsMatch) {
        ...
        if (selfClosing || currentNum !== String(settings.columns)) {
            ...
            if (numAttr)
                tag = tag.replace(/^<w:cols/, `<w:cols${numAttr}`);
        ...
        if (anchor)
            xml = xml.replace(anchor[0], `${anchor[0]}${cols}`);
        else
            xml = xml.replace(/<\/w:sectPr>/, `${cols}</w:sectPr>`);
    }
    return xml;
}
/**
...
function applySectionStartType(sectPrXml, type) {
    ...
    if (/<w:pgSz/.test(xml))
        xml = xml.replace(/(<w:pgSz)/, `${tag}$1`);
    else
        xml = xml.replace(/(<w:sectPr[^>]*>)/, `$1${tag}`);
    return xml;
}
/** Read the page color (w:background) from document.xml; null when unset. */
function readPageColor(parsed) {
