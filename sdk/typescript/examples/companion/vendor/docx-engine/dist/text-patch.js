...
exports.patchParagraphTexts = patchParagraphTexts;
...
function patchParagraphTexts(entryXml, newText, opts = {}) {
    ...
    if (paras.length === 0)
        return null;
    const oldParaTexts = paras.map((p, i) => i === 0 && opts.stripFirstParaLeadingSpace
        ? decodedTextOf(p.xml).replace(/^\s+/, '')
        : decodedTextOf(p.xml));
    if (oldParaTexts.join('\n') === newText)
        return entryXml;
    const newParaTexts = newText.split('\n');
    if (newParaTexts.length !== paras.length)
        return null;
    let out = '';
    let cursor = 0;
    ...
    const check = paragraphSlices(out).map((p, i) => i === 0 && opts.stripFirstParaLeadingSpace
        ? decodedTextOf(p.xml).replace(/^\s+/, '')
        : decodedTextOf(p.xml));
    return check.join('\n') === newText ? out : null;
}
function paragraphSlices(xml) {
    ...
    while ((m = re.exec(xml)) !== null) {
        ...
        if (token === '</w:p>') {
            ...
            if (depth === 0)
                out.push({ start: m.index, end: m.index + token.length, xml: token });
        }
        else {
            if (depth === 0)
                start = m.index;
...
function patchOneParagraph(paraXml, newText, skipLeading) {
    ...
    for (let i = 0; i < slices.length; i++) {
        ...
        const touches = changeStart === changeEnd
    ...
    for (let i = first; i <= last; i++) {
        if (i === first) {
            ...
            newInner.push((0, xml_utils_1.escapeXmlText)(head + replacement + tail));
        }
        else {
            newInner.push('');
        }
