...
exports.RPR_CHILD_ORDER = exports.TABLE_HEADER_FILL = exports.PPR_CHILD_ORDER = exports.LINE_KINDS = void 0;
exports.patchImageParagraphXml = patchImageParagraphXml;
exports.applyImageWrap = applyImageWrap;
exports.patchFieldParagraphXml = patchFieldParagraphXml;
exports.patchMathTokens = patchMathTokens;
exports.patchTableCellTexts = patchTableCellTexts;
exports.patchTextboxParas = patchTextboxParas;
exports.patchTextboxHeights = patchTextboxHeights;
exports.patchTextboxSizes = patchTextboxSizes;
exports.patchShapeStyles = patchShapeStyles;
exports.patchDrawingExtent = patchDrawingExtent;
exports.buildLineParagraphXml = buildLineParagraphXml;
exports.splitXmlChildren = splitXmlChildren;
exports.mergePPrFormat = mergePPrFormat;
exports.stripPPrChange = stripPPrChange;
exports.setPPrChange = setPPrChange;
exports.generateParagraphXml = generateParagraphXml;
exports.generateTableModelXml = generateTableModelXml;
exports.generateTableXml = generateTableXml;
exports.generateTocFieldXml = generateTocFieldXml;
exports.generateCaptionXml = generateCaptionXml;
exports.generateIndexFieldXml = generateIndexFieldXml;
exports.inlineRunsXml = inlineRunsXml;
exports.mergeRPrModel = mergeRPrModel;
exports.buildTextboxParagraphXml = buildTextboxParagraphXml;
exports.buildShapeParagraphXml = buildShapeParagraphXml;
exports.buildWordArtParagraphXml = buildWordArtParagraphXml;
...
function patchImageParagraphXml(xml, patch) {
    ...
    if (patch.rotDeg !== undefined || patch.flipH !== undefined || patch.flipV !== undefined) {
        ...
        out = out.replace(/(<pic:spPr[^>]*>[\s\S]*?)<a:xfrm([^>]*)>/, (_whole, prefix, attrs) => {
            ...
            if (patch.flipH !== undefined)
                setAttr('flipH', patch.flipH ? '1' : null);
            if (patch.flipV !== undefined)
                setAttr('flipV', patch.flipV ? '1' : null);
    ...
    // their original effectExtent (possibly Word-authored shadow/glow padding).
    {
        ...
        if (extM && (touchRot || (touchSize && rotM))) {
            ...
            if (/<wp:effectExtent\b[^>]*\/>/.test(out))
                ...
                out = out.replace(/(<wp:extent\b[^>]*\/?>)/, `$1${ee}`);
        }
    }
    ...
    if (patch.posOffsetX !== undefined) {
        out = out.replace(/(<wp:positionH[^>]*>[\s\S]*?)<wp:posOffset>-?\d+<\/wp:posOffset>([\s\S]*?<\/wp:positionH>)/, `$1<wp:posOffset>${Math.round(patch.posOffsetX)}</wp:posOffset>$2`);
    }
    if (patch.posOffsetY !== undefined) {
        out = out.replace(/(<wp:positionV[^>]*>[\s\S]*?)<wp:posOffset>-?\d+<\/wp:posOffset>([\s\S]*?<\/wp:positionV>)/, `$1<wp:posOffset>${Math.round(patch.posOffsetY)}</wp:posOffset>$2`);
    }
    return out;
}
const WRAP_ELEMENT_RE = /<wp:wrapNone\s*\/>|<wp:wrapSquare[^>]*\/>|<wp:wrapSquare[\s\S]*?<\/wp:wrapSquare>|<wp:wrapTight[\s\S]*?<\/wp:wrapTight>|<wp:wrapThrough[\s\S]*?<\/wp:wrapThrough>|<wp:wrapTopAndBottom\s*\/>|<wp:wrapTopAndBottom[\s\S]*?<\/wp:wrapTopAndBottom>/g;
/**
...
function applyImageWrap(xml, wrap, posOffset, marginAlign) {
    ...
    const keepKind = wrap.startsWith('tight-')
        ...
        : wrap.startsWith('through-')
            ? 'wp:wrapThrough'
            : null;
    const wrapElement = keepKind && existingWrap?.startsWith(`<${keepKind}`)
        ? existingWrap
        : isSide
            ...
            : wrap === 'topBottom'
                ? '<wp:wrapTopAndBottom/>'
                : '<wp:wrapNone/>';
    const anchorOpen = `<wp:anchor distT="0" distB="0" distL="114300" distR="114300" simplePos="0"` +
    ...
    if (/<wp:docPr/.test(out))
        return out.replace(/<wp:docPr/, `${wrapElement}<wp:docPr`);
...
function distributeText(value, nodes) {
    if (nodes.length === 0)
        return [];
    const parts = [];
    let cursor = 0;
...
function replaceTextNodes(xml, replacements) {
    ...
    for (const { node, text } of [...replacements].sort((a, b) => b.node.start - a.node.start)) {
        const replacement = node.open + (0, xml_utils_1.escapeXmlText)(text) + node.close;
 ...
 */
function patchFieldParagraphXml(xml, patch) {
    ...
    if (patch.right !== undefined && tabIndex === -1)
        return xml;
    ...
    if (patch.left !== undefined && leftNodes.length === 0)
        return xml;
    if (patch.right !== undefined && rightNodes.length === 0)
        return xml;
    const replacements = [];
 ...
 */
function patchMathTokens(xml, tokens) {
    const nodes = textNodes(xml, 'm:t');
    if (nodes.length === 0 || nodes.length !== tokens.length)
        return xml;
    return replaceTextNodes(xml, nodes.map((node, i) => ({ node, text: tokens[i] })));
}
// ---- table cell text patching ----
/** top-level segments of `tag` between [from, to), depth-aware for nesting */
function xmlSegments(xml, tag, from, to) {
    ...
    while (i < to) {
        ...
        if (o !== -1 && o < c) {
            ...
            if (gt !== -1 && xml.charAt(gt - 1) === '/') {
                if (depth === 0)
                    segs.push({ start: o, end: gt + 1 }); // self-closing
                i = gt + 1;
                continue;
            }
            if (depth === 0)
            ...
            depth--;
            if (depth === 0)
                segs.push({ start: segStart, end: c + closeTag.length });
            if (depth < 0)
                break;
            i = c + closeTag.length;
        }
 ...
 */
function patchCellXml(tcXml, paras) {
    if (tcXml.indexOf('<w:tbl', 1) !== -1)
        return null;
    ...
    const body = paras
        ...
        return t === ''
            ? `<w:p>${pPr}${keep}</w:p>`
            : `<w:p>${pPr}<w:r>${rPr}<w:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(t)}</w:t></w:r>${keep}</w:p>`;
    })
        .join('');
 ...
 */
function patchTableCellTexts(tableXml, texts) {
    ...
    trSegs.forEach((tr, r) => {
        ...
        tcSegs.forEach((tc, c) => {
            ...
            const patched = Array.isArray(entry)
                ? patchCellXml(tcXml, entry)
                : patchNestedInCell(tcXml, entry.nested);
            if (patched === null)
                return;
...
function patchNestedInCell(tcXml, nested) {
 ...
 */
function patchTxbxContent(segXml, paras) {
 ...
 */
function patchTextboxParas(paragraphXml, boxes) {
    ...
    const inFallback = (pos) => fallbacks.some((f) => pos >= f.start && pos < f.end);
    let out = '';
    let cursor = 0;
    let boxIndex = -1;
...
function patchTextboxHeights(paragraphXml, heightsPx) {
    if (heightsPx.every((height) => height == null))
        return paragraphXml;
    ...
    for (const drawing of drawings) {
        ...
        if (!drawingXml.includes('<w:txbxContent') || paraPlainText(drawingXml) === '')
            continue;
        boxIndex++;
    ...
    resizedXml = resizedXml.replace(/<mc:Fallback>[\s\S]*?<\/mc:Fallback>/g, (fallback) => {
        if (!fallback.includes('<w:txbxContent') || paraPlainText(fallback) === '')
            return fallback;
        fallbackIndex++;
        const heightPx = heightsPx[fallbackIndex];
        if (!heightPx)
            return fallback;
        return fallback.replace(/(style="[^"]*\bheight:)\s*[\d.]+(pt|px)/, (_whole, prefix, unit) => {
            const value = unit === 'pt' ? Math.round(heightPx * 75) / 100 : Math.round(heightPx * 100) / 100;
...
const LINE_SHAPE_RE = /<a:prstGeom[^>]*prst="(?:line|straightConnector1|bentConnector[234]|curvedConnector[234])"/;
/** Same drawing set extractTextboxes yields boxes for (keeps patch indexes aligned) */
function isBoxDrawing(drawingXml) {
    if (drawingXml.includes('<w:txbxContent') && paraPlainText(drawingXml) !== '')
        return true;
...
function patchTextboxSizes(paragraphXml, sizes) {
    ...
    for (const drawing of drawings) {
        const drawingXml = paragraphXml.slice(drawing.start, drawing.end);
        if (!isBoxDrawing(drawingXml))
            continue;
        boxIndex++;
        const size = sizes[boxIndex];
        if (!size || (size.wPx == null && size.hPx == null))
            continue;
    ...
    let fallbackIndex = -1;
    resizedXml = resizedXml.replace(/<mc:Fallback>[\s\S]*?<\/mc:Fallback>/g, (fallback) => {
        if (!isBoxDrawing(fallback) && !fallback.includes('<w:txbxContent'))
            return fallback;
        if (fallback.includes('<w:txbxContent') && paraPlainText(fallback) === '')
            return fallback;
        fallbackIndex++;
        ...
        let next = fallback;
        if (size.hPx != null) {
            next = next.replace(/(style="[^"]*\bheight:)\s*[\d.]+(pt|px)/, (_whole, prefix, unit) => {
                const value = unit === 'pt' ? Math.round(size.hPx * 75) / 100 : Math.round(size.hPx * 100) / 100;
                return `${prefix}${value}${unit}`;
            });
        }
        if (size.wPx != null) {
            next = next.replace(/(style="[^"]*\bwidth:)\s*[\d.]+(pt|px)/, (_whole, prefix, unit) => {
                const value = unit === 'pt' ? Math.round(size.wPx * 75) / 100 : Math.round(size.wPx * 100) / 100;
                return `${prefix}${value}${unit}`;
            });
        }
        return next;
...
function replaceFillSlot(xml, hex) {
    ...
    if (xml.includes('<a:noFill/>'))
        return xml.replace('<a:noFill/>', markup);
    return xml;
}
/** Recolor floating shapes/textboxes/lines (same drawing set as extractTextboxes). */
function patchShapeStyles(paragraphXml, styles) {
    ...
    for (const drawing of drawings) {
        ...
        if (!style || (style.fillHex === undefined && style.borderHex === undefined))
            continue;
        ...
        if (spPrMatch) {
            ...
            if (style.borderHex !== undefined) {
                ...
                if (lnMatch) {
                    ...
                    spPr = spPr.replace('</wps:spPr>', `<a:ln><a:solidFill><a:srgbClr val="${style.borderHex}"/></a:solidFill></a:ln></wps:spPr>`);
                }
            }
...
function patchDrawingExtent(paragraphXml, wPx, hPx) {
...
function buildLineParagraphXml(opts) {
    const def = exports.LINE_KINDS[opts.kind] ?? exports.LINE_KINDS.line;
    const widthEmu = opts.widthEmu ?? 1800000;
    const heightEmu = opts.heightEmu ?? 114300;
    const id = opts.id ?? 1;
    const colorHex = opts.colorHex ?? '000000';
    const ln = `<a:ln w="12700"><a:solidFill><a:srgbClr val="${colorHex}"/></a:solidFill>` +
    ...
    const spPr = `<wps:spPr>` +
        ...
        ln +
        `</wps:spPr>`;
    const wsp = `<wps:wsp xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape">` +
        `<wps:cNvSpPr/>` +
        spPr +
        `<wps:bodyPr/>` +
        `</wps:wsp>`;
    const graphicData = `<a:graphicData xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" ` +
    ...
    const anchor = `<wp:anchor xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" ` +
    ...
    const vmlLine = `<v:line xmlns:v="urn:schemas-microsoft-com:vml" ` +
...
function formatPPrChildren(format) {
    ...
    if (format.pageBreakBefore)
        out.push({ name: 'w:pageBreakBefore', xml: '<w:pageBreakBefore/>' });
    if (format.borders) {
        const line = (side) => `<w:${side} w:val="single" w:sz="4" w:space="1" w:color="auto"/>`;
        const sides = [];
        // schema order inside pBdr: top, left, bottom, right
        if (format.borders.includes('t'))
            sides.push(line('top'));
        if (format.borders.includes('l'))
            sides.push(line('left'));
        if (format.borders.includes('b'))
            sides.push(line('bottom'));
        if (format.borders.includes('r'))
            sides.push(line('right'));
        if (sides.length > 0)
            out.push({ name: 'w:pBdr', xml: `<w:pBdr>${sides.join('')}</w:pBdr>` });
    }
    if (format.shadingFill) {
        out.push({
            name: 'w:shd',
            xml: `<w:shd w:val="clear" w:color="auto" w:fill="${(0, xml_utils_1.escapeXmlAttr)(format.shadingFill)}"/>`,
        });
    }
    if (format.bidi)
        out.push({ name: 'w:bidi', xml: '<w:bidi/>' });
    const spacingAttrs = [];
    ...
    if ((format.lineRule === 'exact' || format.lineRule === 'atLeast') && format.lineRawTwips) {
        ...
        spacingAttrs.push(`w:line="${Math.round(format.lineRawTwips)}"`, `w:lineRule="${format.lineRule}"`);
    }
...
function splitXmlChildren(xml) {
    ...
    while ((match = tagRe.exec(xml)) !== null) {
        ...
        if (closing) {
            depth--;
            if (depth === 0)
                out.push({ name, xml: xml.slice(start, match.index + match[0].length) });
        }
        else if (selfClosing) {
            if (depth === 0)
                out.push({ name: match[2], xml: match[0] });
        }
        else {
            if (depth === 0) {
                start = match.index;
...
function rawSpacingUnchanged(raw, f) {
    ...
    const fAfter = f.spaceAfter !== undefined && f.spaceAfter >= 0 ? Math.round(f.spaceAfter) : undefined;
    if (rawBefore !== fBefore || rawAfter !== fAfter)
        return false;
...
function rawIndUnchanged(raw, f) {
    ...
    const norm = (v) => (v ? Math.round(v) : undefined);
    return (rawLeft === norm(f.indentLeft) &&
...
function rawTabsUnchanged(raw, stops) {
    const rawStops = [];
    if (raw) {
        ...
        for (const kid of splitXmlChildren(inner)) {
            ...
            if (leader && leader !== 'none')
                rawStops[rawStops.length - 1].leader = leader;
        }
    }
    if (rawStops.length !== stops.length)
        return false;
    const normLeader = (l) => (l && l !== 'none' ? l : undefined);
    return rawStops.every((r, i) => r.pos === stops[i].pos &&
        r.val === stops[i].val &&
        normLeader(r.leader) === normLeader(stops[i].leader));
}
function rawFramePrUnchanged(raw, f) {
    const val = rawAttr(raw, 'w:dropCap');
    const rawDc = val === 'drop' || val === 'margin'
...
function pprGroupUnchanged(tag, raw, f) {
    switch (tag) {
 ...
 */
function mergePPrFormat(rawPPr, format) {
    ...
    if (!open) {
        ...
        const sorted = [...fresh].sort((a, b) => exports.PPR_CHILD_ORDER.indexOf(a.name) - exports.PPR_CHILD_ORDER.indexOf(b.name));
    ...
    if (format?.tabStops !== undefined)
        managedTags.add('w:tabs');
    if (format?.dropCap !== undefined)
        managedTags.add('w:framePr');
    const rawChildren = splitXmlChildren(inner);
    const rawOf = (tag) => rawChildren.find((c) => c.name === tag)?.xml;
    const rebuilt = new Set([...managedTags].filter((tag) => !pprGroupUnchanged(tag, rawOf(tag), format ?? {})));
    const freshOut = fresh.filter((c) => rebuilt.has(c.name));
    const kept = rawChildren.filter((c) => !rebuilt.has(c.name));
    const rank = (n) => exports.PPR_CHILD_ORDER.indexOf(n);
    const parts = [];
    let fi = 0;
    let prevRank = -1;
 ...
 */
function stripPPrChange(rawPPr) {
...
function revisionPPrChangeXml(changeJson) {
    ...
    if (old.styleId) {
        children.push({
            name: 'w:pStyle',
            xml: `<w:pStyle w:val="${(0, xml_utils_1.escapeXmlAttr)(String(old.styleId))}"/>`,
        });
    }
    if (old.type === 'docListItem' && old.numId) {
        ...
        children.push({
            name: 'w:numPr',
            xml: `<w:numPr><w:ilvl w:val="${ilvl}"/><w:numId w:val="${(0, xml_utils_1.escapeXmlAttr)(String(old.numId))}"/></w:numPr>`,
        });
    }
    children.push(...formatPPrChildren(oldFormat));
    children.sort((a, b) => exports.PPR_CHILD_ORDER.indexOf(a.name) - exports.PPR_CHILD_ORDER.indexOf(b.name));
    const attrs = ` w:id="${(0, xml_utils_1.escapeXmlAttr)(String(change.id ?? '0'))}"` +
        ` w:author="${(0, xml_utils_1.escapeXmlAttr)(String(change.author ?? ''))}"` +
        (change.date ? ` w:date="${(0, xml_utils_1.escapeXmlAttr)(String(change.date))}"` : '');
 ...
 */
function setPPrChange(rawPPr, changeJson) {
    const change = revisionPPrChangeXml(changeJson);
    if (!change)
        return rawPPr;
    const clean = stripPPrChange(rawPPr);
    if (!clean || clean === '<w:pPr/>')
        return `<w:pPr>${change}</w:pPr>`;
...
function generateParagraphXml(block, ctx) {
    const crossStarts = (block.commentStarts ?? [])
        .map((id) => `<w:commentRangeStart w:id="${(0, xml_utils_1.escapeXmlAttr)(id)}"/>`)
        .join('');
    const crossEnds = (block.commentEnds ?? [])
        .map((id) => `<w:commentRangeEnd w:id="${(0, xml_utils_1.escapeXmlAttr)(id)}"/>` +
        `<w:r><w:commentReference w:id="${(0, xml_utils_1.escapeXmlAttr)(id)}"/></w:r>`)
        .join('');
    const content = bookmarksXml(block.hiddenBookmarks) +
        bookmarksXml(block.bookmarks) +
        crossStarts +
        generateRunsXml(block.runs, ctx) +
        crossEnds;
    if (block.rawPPr !== undefined)
        return `<w:p>${block.rawPPr}${content}</w:p>`;
    const children = [];
    let styleId;
    ...
    if (styleId)
        children.push({ name: 'w:pStyle', xml: `<w:pStyle w:val="${(0, xml_utils_1.escapeXmlAttr)(styleId)}"/>` });
    if (block.type === 'listItem' && block.list) {
        const ilvl = Math.min(Math.max(block.list.ilvl, 0), 8);
        children.push({
            name: 'w:numPr',
            xml: `<w:numPr><w:ilvl w:val="${ilvl}"/><w:numId w:val="${(0, xml_utils_1.escapeXmlAttr)(block.list.numId)}"/></w:numPr>`,
        });
    }
    ...
    if (block.pPrChange) {
        ...
        if (revision)
            children.push({ name: 'w:pPrChange', xml: revision });
    }
    children.sort((a, b) => exports.PPR_CHILD_ORDER.indexOf(a.name) - exports.PPR_CHILD_ORDER.indexOf(b.name));
...
function bookmarkIdOf(name) {
    let h = 0;
    for (let i = 0; i < name.length; i++)
        h = (h * 31 + name.charCodeAt(i)) | 0;
    return Math.abs(h) % 0x7fffffff;
}
function bookmarksXml(names) {
    if (!names?.length)
        return '';
    return names
        .map((name) => {
        const id = bookmarkIdOf(name);
        return `<w:bookmarkStart w:id="${id}" w:name="${(0, xml_utils_1.escapeXmlAttr)(name)}"/><w:bookmarkEnd w:id="${id}"/>`;
    })
        .join('');
}
/** shading fill used for generated table header rows (hex without '#') */
exports.TABLE_HEADER_FILL = 'F2F2F2';
...
function cellBordersXml(borders) {
    const side = (name) => {
        ...
        const color = b.style === 'none' || b.style === 'nil'
            ? ''
            : ` w:color="${(0, xml_utils_1.escapeXmlAttr)(b.color ?? 'auto')}"`;
        return `<w:${name} w:val="${(0, xml_utils_1.escapeXmlAttr)(b.style)}"${sz}${color}/>`;
    };
...
function tableCellXml(cell, width, verticalMerge) {
    ...
    setTcPrChild(children, 'w:gridSpan', cell.colSpan && cell.colSpan > 1 ? `<w:gridSpan w:val="${cell.colSpan}"/>` : null);
    const merge = verticalMerge ?? cell.vMerge;
    setTcPrChild(children, 'w:vMerge', merge === 'restart'
    ...
    // uses explicit none
    if (cell.borders)
        setTcPrChild(children, 'w:tcBorders', cellBordersXml(cell.borders));
    setTcPrChild(children, 'w:shd', cell.fill ? `<w:shd w:val="clear" w:color="auto" w:fill="${(0, xml_utils_1.escapeXmlAttr)(cell.fill)}"/>` : null);
    setTcPrChild(children, 'w:vAlign', cell.vAlign && cell.vAlign !== 'top' ? `<w:vAlign w:val="${cell.vAlign}"/>` : null);
    ...
    const paraXmls = paragraphs.map((paragraph) => {
        ...
        const numPr = list
            ? `<w:numPr><w:ilvl w:val="${list.ilvl}"/><w:numId w:val="${(0, xml_utils_1.escapeXmlAttr)(list.numId)}"/></w:numPr>`
            : '';
...
function generateTableModelXml(model, originalTableXml) {
    const columnCount = Math.max(1, model.colWidthsPct?.length ?? 0, ...model.rows.map((row) => row.reduce((sum, cell) => sum + (cell.colSpan ?? 1), 0)));
    const percentages = model.colWidthsPct?.length === columnCount
        ? model.colWidthsPct
        : Array.from({ length: columnCount }, () => 100 / columnCount);
    const totalPct = percentages.reduce((sum, value) => sum + value, 0) || 100;
    const widths = model.colWidthsTwips?.length === columnCount
    ...
    const rows = model.rows
        ...
        if (h != null && h > 0) {
            ...
            if (!trPr)
                trPr = `<w:trPr>${tag}</w:trPr>`;
            else if (/<w:trHeight[^>]*\/>/.test(trPr))
                trPr = trPr.replace(/<w:trHeight[^>]*\/>/, tag);
            else
                trPr = trPr.replace('</w:trPr>', `${tag}</w:trPr>`);
        }
        else if (trPr) {
            trPr = trPr.replace(/<w:trHeight[^>]*\/>/, '');
            if (/^<w:trPr(?:\s[^>]*)?>\s*<\/w:trPr>$/.test(trPr))
                trPr = '';
        }
        return `<w:tr>${trPr}${cells.join('')}</w:tr>`;
    })
        .join('');
    const originalTblPr = originalTableXml?.match(/<w:tblPr(?:\s[^>]*)?>[\s\S]*?<\/w:tblPr>|<w:tblPr(?:\s[^>]*)?\/>/)?.[0];
    let tblPr = originalTblPr ??
    ...
    if (model.tblStyleId !== undefined) {
        ...
        if (model.tblStyleId !== '') {
            const tag = `<w:tblStyle w:val="${(0, xml_utils_1.escapeXmlAttr)(model.tblStyleId)}"/>`;
            ...
            if (!/<w:tblLook[\s/>]/.test(tblPr)) {
                tblPr = tblPr.replace('</w:tblPr>', '<w:tblLook w:val="04A0" w:firstRow="1" w:lastRow="0" w:firstColumn="1" w:lastColumn="0" w:noHBand="0" w:noVBand="1"/></w:tblPr>');
            }
        }
    }
 ...
 */
function generateTableXml(rows, cols, opts = {}) {
    ...
    const cell = (header) => `<w:tc><w:tcPr><w:tcW w:w="${colWidth}" w:type="dxa"/>` +
        (header ? `<w:shd w:val="clear" w:color="auto" w:fill="${exports.TABLE_HEADER_FILL}"/>` : '') +
        '</w:tcPr>' +
        (header ? '<w:p><w:r><w:rPr><w:b/></w:rPr></w:r></w:p>' : '<w:p/>') +
        '</w:tc>';
    const row = (header) => `<w:tr>${cell(header).repeat(safeCols)}</w:tr>`;
 ...
 */
function generateTocFieldXml(entries) {
    if (entries.length === 0)
        return [];
    const maxLevel = Math.min(Math.max(...entries.map((e) => e.level), 1), 9);
    const pPr = (level) => `<w:pPr><w:pStyle w:val="TOC${Math.min(Math.max(level, 1), 9)}"/>` +
        '<w:tabs><w:tab w:val="right" w:leader="dot" w:pos="9350"/></w:tabs>' +
        '<w:rPr><w:noProof/></w:rPr></w:pPr>';
    const entryRuns = (text, pageNo) => `<w:r><w:rPr><w:noProof/></w:rPr><w:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(text)}</w:t></w:r>` +
        '<w:r><w:rPr><w:noProof/></w:rPr><w:tab/></w:r>' +
        (pageNo !== undefined ? `<w:r><w:rPr><w:noProof/></w:rPr><w:t>${pageNo}</w:t></w:r>` : '');
    const begin = '<w:r><w:fldChar w:fldCharType="begin" w:dirty="true"/></w:r>' +
 ...
 */
function generateCaptionXml(label, number, text) {
    const rPr = '<w:rPr><w:color w:val="44546A"/><w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr>';
    const run = (inner) => `<w:r>${rPr}${inner}</w:r>`;
    return ('<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:before="80" w:after="200"/></w:pPr>' +
        run(`<w:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(label)} </w:t>`) +
        run('<w:fldChar w:fldCharType="begin" w:dirty="true"/>') +
        run(`<w:instrText xml:space="preserve"> SEQ ${(0, xml_utils_1.escapeXmlText)(label)} \\* ARABIC </w:instrText>`) +
        run('<w:fldChar w:fldCharType="separate"/>') +
        run(`<w:t>${number}</w:t>`) +
        run('<w:fldChar w:fldCharType="end"/>') +
        (text ? run(`<w:t xml:space="preserve"> ${(0, xml_utils_1.escapeXmlText)(text)}</w:t>`) : '') +
        '</w:p>');
}
/**
 ...
 */
function generateIndexFieldXml(terms) {
    ...
    const begin = '<w:r><w:fldChar w:fldCharType="begin" w:dirty="true"/></w:r>' +
        ...
        '<w:r><w:fldChar w:fldCharType="separate"/></w:r>';
    const end = '<w:r><w:fldChar w:fldCharType="end"/></w:r>';
    const pPr = '<w:pPr><w:tabs><w:tab w:val="right" w:leader="dot" w:pos="4300"/></w:tabs>' +
        '<w:rPr><w:noProof/></w:rPr></w:pPr>';
    return unique.map((term, i) => {
        const first = i === 0 ? begin : '';
        const last = i === unique.length - 1 ? end : '';
        return (`<w:p>${pPr}${first}` +
            `<w:r><w:rPr><w:noProof/></w:rPr><w:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(term)}</w:t></w:r>` +
            '<w:r><w:rPr><w:noProof/></w:rPr><w:tab/></w:r>' +
            `${last}</w:p>`);
    });
}
...
function runsXml(runs, allocate) {
    ...
    const startsAt = (i) => [...firstOf]
        .filter(([, at]) => at === i)
        .map(([id]) => `<w:commentRangeStart w:id="${(0, xml_utils_1.escapeXmlAttr)(id)}"/>`)
        .join('');
    const endsAt = (i) => [...lastOf]
        .filter(([, at]) => at === i)
        .map(([id]) => `<w:commentRangeEnd w:id="${(0, xml_utils_1.escapeXmlAttr)(id)}"/>` +
        `<w:r><w:commentReference w:id="${(0, xml_utils_1.escapeXmlAttr)(id)}"/></w:r>`)
        .join('');
        ...
        while (i < to) {
            ...
            if (run.link) {
                ...
                for (let j = groupStart; j < i; j++)
                    parts.push(startsAt(j));
                const tooltip = group[0]?.link?.tooltip;
                const tipAttr = tooltip ? ` w:tooltip="${(0, xml_utils_1.escapeXmlAttr)(tooltip)}"` : '';
                if (href.startsWith('#')) {
                    const inner = group.map((r) => runFragmentXml(r, true)).join('');
                    parts.push(`<w:hyperlink w:anchor="${(0, xml_utils_1.escapeXmlAttr)(href.slice(1))}"${tipAttr}>${inner}</w:hyperlink>`);
                }
                else {
                    const finalRId = rId ?? allocate?.(href);
                    if (finalRId) {
                        const inner = group.map((r) => runFragmentXml(r, true)).join('');
                        parts.push(`<w:hyperlink r:id="${(0, xml_utils_1.escapeXmlAttr)(finalRId)}"${tipAttr}>${inner}</w:hyperlink>`);
                    }
                    else {
    ...
    const revKey = (r) => r.ins || r.del
        ...
        : '';
    let revSeq = 9001;
    const revAttrs = (info) => ` w:id="${(0, xml_utils_1.escapeXmlAttr)(info.id ?? String(revSeq++))}"` +
        ` w:author="${(0, xml_utils_1.escapeXmlAttr)(info.author)}"` +
        (info.date ? ` w:date="${(0, xml_utils_1.escapeXmlAttr)(info.date)}"` : '');
    let g = 0;
    while (g < runs.length) {
        const key = revKey(runs[g]);
        let end = g;
        while (end < runs.length && revKey(runs[end]) === key)
            end++;
        if (key === '') {
            emitRange(g, end);
        }
            ...
            if (ins)
                parts.push(`<w:ins${revAttrs(ins)}>`);
            if (del)
                parts.push(`<w:del${revAttrs(del)}>`);
            emitRange(g, end);
            if (del)
                parts.push('</w:del>');
            if (ins)
 ...
 */
function runFragmentXml(run, insideLink) {
    ...
    if (run.noteRef) {
        ...
        return ('<w:r><w:rPr><w:vertAlign w:val="superscript"/></w:rPr>' +
            `<${tag} w:id="${(0, xml_utils_1.escapeXmlAttr)(run.noteRef.id)}"/></w:r>`);
    }
    if (run.refField !== undefined) {
        ...
        return ('<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
            `<w:r><w:instrText xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(instr)}</w:instrText></w:r>` +
            '<w:r><w:fldChar w:fldCharType="separate"/></w:r>' +
    ...
    if (run.instrField !== undefined) {
        ...
        return ('<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
            `<w:r><w:instrText xml:space="preserve"> ${(0, xml_utils_1.escapeXmlText)(run.instrField)} </w:instrText></w:r>` +
            '<w:r><w:fldChar w:fldCharType="separate"/></w:r>' +
    ...
    if (run.xeTerm !== undefined) {
        ...
        xml +=
            '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
                `<w:r><w:instrText xml:space="preserve"> XE "${(0, xml_utils_1.escapeXmlText)(term)}" </w:instrText></w:r>` +
                '<w:r><w:fldChar w:fldCharType="end"/></w:r>';
    }
    return xml;
}
/** CT_RPr child sequence (subset), for schema-ordered assembly and merging */
exports.RPR_CHILD_ORDER = [
    'w:rStyle',
    'w:rFonts',
    'w:b',
    'w:bCs',
...
const rawAttr = (xml, attr) => xml ? new RegExp(` ${attr}="([^"]*)"`).exec(xml)?.[1] : undefined;
...
function freshRFontsXml(font, fontAscii) {
    const a = (0, xml_utils_1.escapeXmlAttr)(fontAscii ?? font ?? '');
    const ea = font ? ` w:eastAsia="${(0, xml_utils_1.escapeXmlAttr)(font)}"` : '';
 ...
 */
function mergeRFontsXml(rawXml, run) {
    const attrs = new Map();
    for (const m of rawXml.matchAll(/ ([\w:]+)="([^"]*)"/g))
        attrs.set(m[1], m[2]);
    ...
    const set = (slot, theme, value) => {
        attrs.set(slot, (0, xml_utils_1.escapeXmlAttr)(value));
        attrs.delete(theme);
    };
    if (run.fontAscii) {
...
function revisionRPrChangeXml(run) {
    ...
    const props = [];
    if (old.styleId)
        props.push(`<w:rStyle w:val="${(0, xml_utils_1.escapeXmlAttr)(old.styleId)}"/>`);
    if (old.font || old.fontAscii)
        props.push(freshRFontsXml(old.font, old.fontAscii));
    if (old.bold)
        props.push('<w:b/>');
    if (old.italic)
        props.push('<w:i/>');
    if (old.strike)
        props.push('<w:strike/>');
    if (old.color)
        props.push(`<w:color w:val="${(0, xml_utils_1.escapeXmlAttr)(old.color)}"/>`);
    if (old.charSpacingTwips)
        props.push(`<w:spacing w:val="${old.charSpacingTwips}"/>`);
    if (old.charScalePct)
        props.push(`<w:w w:val="${old.charScalePct}"/>`);
    if (old.sizeHalfPoints) {
        props.push(`<w:sz w:val="${old.sizeHalfPoints}"/>`);
        props.push(`<w:szCs w:val="${old.sizeHalfPoints}"/>`);
    }
    if (old.highlight)
        props.push(`<w:highlight w:val="${(0, xml_utils_1.escapeXmlAttr)(old.highlight)}"/>`);
    if (old.underline)
        props.push('<w:u w:val="single"/>');
    if (old.vertAlign)
        props.push(`<w:vertAlign w:val="${old.vertAlign}"/>`);
    const attrs = ` w:id="${(0, xml_utils_1.escapeXmlAttr)(change.id ?? '0')}"` +
        ` w:author="${(0, xml_utils_1.escapeXmlAttr)(change.author)}"` +
        (change.date ? ` w:date="${(0, xml_utils_1.escapeXmlAttr)(change.date)}"` : '');
...
function modelRPrChildren(run, insideLink) {
    const out = [];
    if (insideLink)
        out.push({ name: 'w:rStyle', xml: '<w:rStyle w:val="Hyperlink"/>' });
    else if (run.styleId)
        out.push({ name: 'w:rStyle', xml: `<w:rStyle w:val="${(0, xml_utils_1.escapeXmlAttr)(run.styleId)}"/>` });
    if (run.font || run.fontAscii) {
        out.push({ name: 'w:rFonts', xml: freshRFontsXml(run.font, run.fontAscii) });
    }
    if (run.bold)
        out.push({ name: 'w:b', xml: '<w:b/>' });
    if (run.italic)
        out.push({ name: 'w:i', xml: '<w:i/>' });
    if (run.strike)
        out.push({ name: 'w:strike', xml: '<w:strike/>' });
    if (run.color)
        out.push({ name: 'w:color', xml: `<w:color w:val="${(0, xml_utils_1.escapeXmlAttr)(run.color)}"/>` });
    ...
    if (run.highlight)
        out.push({ name: 'w:highlight', xml: `<w:highlight w:val="${(0, xml_utils_1.escapeXmlAttr)(run.highlight)}"/>` });
    if (run.underline)
        out.push({ name: 'w:u', xml: '<w:u w:val="single"/>' });
    if (run.vertAlign)
        out.push({ name: 'w:vertAlign', xml: `<w:vertAlign w:val="${run.vertAlign}"/>` });
    const rPrChange = revisionRPrChangeXml(run);
    if (rPrChange)
        out.push({ name: 'w:rPrChange', xml: rPrChange });
    return out;
}
/**
 ...
 */
function mergeRPrModel(rawRPr, run, insideLink) {
    ...
    const rank = (n) => exports.RPR_CHILD_ORDER.indexOf(n);
    const parts = [];
    let fi = 0;
    let prevRank = -1;
    for (const child of kept) {
 ...
 */
function buildTextboxParagraphXml(opts) {
    const widthEmu = opts?.widthEmu ?? 1800000;
    ...
    const mcNs = 'xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" ' +
        'xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape"';
    const mcChoice = `<mc:Choice Requires="wps">` + `<w:drawing>${anchor}</w:drawing>` + `</mc:Choice>`;
    const vmlRect = `<v:rect xmlns:v="urn:schemas-microsoft-com:vml" style="position:absolute;width:${widthEmu / EMU_PER_PT}pt;height:${heightEmu / EMU_PER_PT}pt" filled="t" stroked="t">` +
    ...
    const alternateContent = `<mc:AlternateContent ${mcNs}>` + mcChoice + mcFallback + `</mc:AlternateContent>`;
 ...
 */
function buildShapeParagraphXml(opts) {
    const widthEmu = opts.widthEmu ?? 1800000;
    const heightEmu = opts.heightEmu ?? 1080000;
    const id = opts.id ?? 1;
    ...
    const anchor = `<wp:anchor xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" ` +
        ...
        graphic +
        `</wp:anchor>`;
    const mcNs2 = 'xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" ' +
        'xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape"';
    const mcChoice = `<mc:Choice Requires="wps">` + `<w:drawing>${anchor}</w:drawing>` + `</mc:Choice>`;
    const vmlRect = `<v:rect xmlns:v="urn:schemas-microsoft-com:vml" style="position:absolute;width:${widthEmu / EMU_PER_PT}pt;height:${heightEmu / EMU_PER_PT}pt" filled="t" stroked="t">` +
        (opts.withTextbox
        ...
        `</v:rect>`;
    const mcFallback = `<mc:Fallback>` + `<w:pict>${vmlRect}</w:pict>` + `</mc:Fallback>`;
    const alternateContent = `<mc:AlternateContent ${mcNs2}>` + mcChoice + mcFallback + `</mc:AlternateContent>`;
    return `<w:p><w:r>${alternateContent}</w:r></w:p>`;
}
/**
 ...
 */
function buildWordArtParagraphXml(opts) {
    ...
    const rPr = `<w:rPr>` +
        ...
        `<w:szCs w:val="72"/>` +
        `</w:rPr>`;
    const textRun = `<w:r>${rPr}<w:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(text)}</w:t></w:r>`;
    ...
    const mcNs = 'xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" ' +
        'xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape"';
    const mcChoice = `<mc:Choice Requires="wps">` + `<w:drawing>${anchor}</w:drawing>` + `</mc:Choice>`;
    const vmlRect = `<v:rect xmlns:v="urn:schemas-microsoft-com:vml" style="position:absolute;width:${widthEmu / EMU_PER_PT}pt;height:${heightEmu / EMU_PER_PT}pt" filled="f" stroked="f">` +
        `<v:textbox><w:txbxContent><w:p><w:r>${rPr}<w:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(text)}</w:t></w:r></w:p></w:txbxContent></v:textbox>` +
        `</v:rect>`;
    const mcFallback = `<mc:Fallback><w:pict>${vmlRect}</w:pict></mc:Fallback>`;
    const alternateContent = `<mc:AlternateContent ${mcNs}>` + mcChoice + mcFallback + `</mc:AlternateContent>`;
    return `<w:p><w:r>${alternateContent}</w:r></w:p>`;
}
function generateRunXml(run, insideLink) {
    ...
    const flush = () => {
        if (buffer !== '') {
            segments.push(`<${textTag} xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(buffer)}</${textTag}>`);
            buffer = '';
        }
    };
