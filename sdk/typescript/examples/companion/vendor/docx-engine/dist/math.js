...
exports.ommlFragmentsOf = ommlFragmentsOf;
exports.mathTokensOf = mathTokensOf;
exports.ommlToMathML = ommlToMathML;
exports.ommlToLatex = ommlToLatex;
exports.latexToOmml = latexToOmml;
exports.mathParagraphXml = mathParagraphXml;
...
function ommlToMathML(ommlXml) {
    ...
    for (const source of sources) {
        let parsed;
        try {
            parsed = xml_utils_1.xmlParser.parse(source);
        }
        catch {
            continue;
        }
        const root = parsed.find((n) => (0, xml_utils_1.nameOf)(n) === 'm:oMath');
        if (!root)
            continue;
        const body = mmlChildren((0, xml_utils_1.childrenOf)(root));
        if (body)
            parts.push(`<math display="block"><mrow>${body}</mrow></math>`);
    }
    return parts.join('');
}
/** child elements that are content (skip m:*Pr property bags and #text glue) */
function contentChildren(node) {
    return (0, xml_utils_1.childrenOf)(node).filter((c) => {
        const name = (0, xml_utils_1.nameOf)(c);
...
function mmlSlot(parent, name) {
    const slot = (0, xml_utils_1.findChild)(parent, name);
    if (!slot)
        return '<mrow></mrow>';
    return `<mrow>${mmlChildren(contentChildren(slot))}</mrow>`;
}
function propVal(node, prName, childName) {
    const pr = (0, xml_utils_1.findChild)(node, prName);
    if (!pr)
        return undefined;
    const child = (0, xml_utils_1.findChild)(pr, childName);
    if (!child)
        return undefined;
    return (0, xml_utils_1.attrsOf)(child)['m:val'];
}
...
function mo(ch, extra = '') {
    return `<mo${extra}>${(0, xml_utils_1.escapeXmlText)(ch)}</mo>`;
}
function mmlOf(node) {
    switch ((0, xml_utils_1.nameOf)(node)) {
        case 'm:r':
            return runToMml(node);
        case 'm:f': {
            const type = propVal(node, 'm:fPr', 'm:type');
            const attrs = type === 'noBar'
                ? ' linethickness="0"'
                : type === 'lin' || type === 'skw'
                    ? ' bevelled="true"'
                    : '';
        ...
        case 'm:rad': {
            ...
            if (propOn(node, 'm:radPr', 'm:degHide') || !(0, xml_utils_1.findChild)(node, 'm:deg')) {
        ...
        case 'm:d': {
            ...
            const slots = (0, xml_utils_1.childrenOf)(node).filter((c) => (0, xml_utils_1.nameOf)(c) === 'm:e');
            const body = slots
        ...
        case 'm:m': {
            const rows = (0, xml_utils_1.childrenOf)(node)
                .filter((c) => (0, xml_utils_1.nameOf)(c) === 'm:mr')
                .map((row) => {
                const cells = (0, xml_utils_1.childrenOf)(row)
                    .filter((c) => (0, xml_utils_1.nameOf)(c) === 'm:e')
        ...
        case 'm:t':
            return runTextToMml((0, xml_utils_1.textOf)(node), false);
        case undefined:
            return '';
        default:
...
function runToMml(run) {
    const sty = propVal(run, 'm:rPr', 'm:sty');
    const plain = sty === 'p' || !!(0, xml_utils_1.findChild)((0, xml_utils_1.findChild)(run, 'm:rPr') ?? {}, 'm:nor');
    let out = '';
    for (const child of (0, xml_utils_1.childrenOf)(run)) {
        if ((0, xml_utils_1.nameOf)(child) === 'm:t')
            out += runTextToMml((0, xml_utils_1.textOf)(child), plain);
    }
    return out;
}
...
function runTextToMml(text, plain) {
    if (plain)
        return text === '' ? '' : `<mi>${(0, xml_utils_1.escapeXmlText)(text)}</mi>`;
    let out = '';
    let i = 0;
    const chars = [...text];
    while (i < chars.length) {
        ...
        if (/[0-9.]/.test(ch)) {
            ...
            out += `<mi>${(0, xml_utils_1.escapeXmlText)(ch)}</mi>`;
            i++;
        }
        else if (ch === ' ') {
            i++;
            ...
            out += `<mtext>${(0, xml_utils_1.escapeXmlText)(ch)}</mtext>`;
            i++;
        }
    }
    return out;
...
const matrixEnvs = lazyMap(() => {
    ...
    for (const [env, delims] of Object.entries(MATRIX_DELIMS)) {
        if (env !== 'matrix' && delims)
            map.set(`${delims.beg} ${delims.end}`, env);
    }
    return map;
});
/**
...
function ommlToLatex(ommlXml) {
    ...
    let parsed;
    try {
        parsed = xml_utils_1.xmlParser.parse(sources[0]);
    }
    catch {
        return null;
    }
    ...
    if (!root)
        return null;
    try {
        return latexOfChildren((0, xml_utils_1.childrenOf)(root))
            .trim()
            .replace(/\s{2,}/g, ' ');
    }
    catch (e) {
        if (e instanceof UnsupportedOmml)
            return null;
        throw e;
    }
}
function latexOfChildren(nodes) {
    return nodes
        .filter((n) => !!(0, xml_utils_1.nameOf)(n))
        .map(latexOfNode)
        .join('');
}
...
function latexOfNode(node) {
    switch ((0, xml_utils_1.nameOf)(node)) {
        ...
        case 'm:t':
            return charsToLatex((0, xml_utils_1.textOf)(node));
        case 'm:f': {
            const type = propVal(node, 'm:fPr', 'm:type');
        ...
        case 'm:func': {
            const name = plainTextOfRuns((0, xml_utils_1.findChild)(node, 'm:fName')).trim();
        ...
        case 'm:limLow': {
            const base = plainTextOfRuns((0, xml_utils_1.findChild)(node, 'm:e')).trim();
            if (base === 'lim')
                return `\\lim_{${latexSlot(node, 'm:lim')}}`;
            throw new UnsupportedOmml();
        }
        case 'm:acc': {
            const chr = propVal(node, 'm:accPr', 'm:chr') ?? '̂';
        ...
        case 'm:groupChr': {
            const chr = propVal(node, 'm:groupChrPr', 'm:chr') ?? '⏟';
            if (chr === '⏟')
                return `\\underbrace{${latexSlot(node, 'm:e')}}`;
            if (chr === '⏞')
                return `\\overbrace{${latexSlot(node, 'm:e')}}`;
            throw new UnsupportedOmml();
        }
        case 'm:m':
...
function matrixBody(node) {
    const rowName = (0, xml_utils_1.findChildren)(node, 'm:mr').length > 0 ? 'm:mr' : 'm:e';
    const rows = (0, xml_utils_1.childrenOf)(node)
        .filter((c) => (0, xml_utils_1.nameOf)(c) === rowName)
        .map((row) => rowName === 'm:mr'
        ? (0, xml_utils_1.childrenOf)(row)
            .filter((c) => (0, xml_utils_1.nameOf)(c) === 'm:e')
...
function delimiterToLatex(node) {
    ...
    if (beg === '(' &&
        ...
        inner.length === 1 &&
        (0, xml_utils_1.nameOf)(inner[0]) === 'm:f' &&
    ...
    if (inner.length === 1 && ((0, xml_utils_1.nameOf)(inner[0]) === 'm:m' || (0, xml_utils_1.nameOf)(inner[0]) === 'm:eqArr')) {
        const env = matrixEnvs().get(`${beg} ${end}`);
        if (env)
            return `\\begin{${env}} ${matrixBody(inner[0])} \\end{${env}}`;
    }
    const begTok = delimTokens().get(beg);
    const endTok = delimTokens().get(end);
    if (begTok === undefined || endTok === undefined)
        throw new UnsupportedOmml();
...
function plainTextOfRuns(node) {
    if (!node)
        return '';
    let out = '';
    for (const run of (0, xml_utils_1.findChildren)(node, 'm:r')) {
        for (const child of (0, xml_utils_1.childrenOf)(run)) {
            if ((0, xml_utils_1.nameOf)(child) === 'm:t')
                out += (0, xml_utils_1.textOf)(child);
        }
    }
    return out;
}
function runToLatex(run) {
    const sty = propVal(run, 'm:rPr', 'm:sty');
    const plain = sty === 'p' || !!(0, xml_utils_1.findChild)((0, xml_utils_1.findChild)(run, 'm:rPr') ?? {}, 'm:nor');
    let text = '';
    for (const child of (0, xml_utils_1.childrenOf)(run)) {
        if ((0, xml_utils_1.nameOf)(child) === 'm:t')
            text += (0, xml_utils_1.textOf)(child);
    }
    if (!plain)
        return charsToLatex(text);
    const trimmed = text.trim();
    if (trimmed === '')
        return ' ';
    if (LATEX_FUNCTIONS.has(trimmed))
        return `\\${trimmed} `;
    if (trimmed === 'lim')
        return '\\lim ';
    if (/[{}\\]/.test(text))
...
function mathParagraphXml(omml, align = 'center') {
    const jc = align === 'center' ? '' : `<w:pPr><w:jc w:val="${align}"/></w:pPr>`;
    return (`<w:p>${jc}<m:oMathPara><m:oMathParaPr><m:jc m:val="${align === 'center' ? 'center' : align}"/></m:oMathParaPr>` +
...
function mathRun(text, plain = false) {
    ...
    return `<m:r>${rPr}<m:t xml:space="preserve">${(0, xml_utils_1.escapeXmlText)(text)}</m:t></m:r>`;
}
function peek(p) {
    return p.src[p.pos] ?? '';
}
...
function parseGroup(p) {
    ...
    const ch = peek(p);
    if (ch === '' || '{}^_&'.includes(ch))
        throw new LatexError('An argument is required here');
    p.pos++;
    return mathRun(ch);
}
/** raw text of a {...} group (for \text / \begin names) */
function readBraceText(p) {
    skipSpaces(p);
    if (peek(p) !== '{')
        throw new LatexError('Expected { here');
    p.pos++;
    let depth = 1;
    let out = '';
...
function parseSequence(p, stop) {
    const atoms = [];
    for (;;) {
        skipSpaces(p);
        if (p.pos >= p.src.length || stop())
            break;
        const ch = peek(p);
        if (ch === '^' || ch === '_') {
            p.pos++;
            ...
            if ((other === '^' || other === '_') && other !== ch) {
                ...
                atoms.push(`<m:sSubSup><m:e>${base}</m:e><m:sub>${sub}</m:sub><m:sup>${sup}</m:sup></m:sSubSup>`);
            }
            else if (ch === '^') {
...
function parseAtom(p) {
    ...
    if (ch === '{')
        return parseGroup(p);
    if (ch === '}')
        throw new LatexError('Unexpected }');
    if (ch === '\\') {
        p.pos++;
        return parseControl(p);
    }
    ...
    if (text === '')
        throw new LatexError(`Cannot parse: "${ch}"`);
...
function naryOmml(p, chr, limLoc) {
    ...
    const pr = `<m:naryPr><m:chr m:val="${(0, xml_utils_1.escapeXmlAttr)(chr)}"/><m:limLoc m:val="${limLoc}"/>` +
...
function matrixOmml(p, env) {
    ...
    while (!done) {
        ...
        if (peek(p) === '&') {
            ...
            if (closing !== env)
                throw new LatexError(`\\end{${closing}} does not match \\begin{${env}}`);
            done = true;
        }
        else {
    ...
    if (!delims)
        return matrix;
    return ('<m:d><m:dPr>' +
        `<m:begChr m:val="${(0, xml_utils_1.escapeXmlAttr)(delims.beg)}"/>` +
        `<m:endChr m:val="${(0, xml_utils_1.escapeXmlAttr)(delims.end)}"/>` +
        `</m:dPr><m:e>${matrix}</m:e></m:d>`);
}
const LEFT_RIGHT_CHARS = {
    '(': '(',
    ')': ')',
    '[': '[',
    ']': ']',
...
function readDelimiter(p) {
    ...
    if (peek(p) === '\\') {
        ...
        const key = '\\' + name;
        if (key in LEFT_RIGHT_CHARS)
            return LEFT_RIGHT_CHARS[key];
        p.pos = start;
...
function parseControl(p) {
    const name = readControlName(p);
    if (name in LATEX_SYMBOLS)
        return mathRun(LATEX_SYMBOLS[name]);
    if (name in NARY_OPS)
        return naryOmml(p, NARY_OPS[name].chr, NARY_OPS[name].limLoc);
    if (name in ACCENT_CHARS) {
        const base = parseGroup(p);
        return `<m:acc><m:accPr><m:chr m:val="${(0, xml_utils_1.escapeXmlAttr)(ACCENT_CHARS[name])}"/></m:accPr><m:e>${base}</m:e></m:acc>`;
    }
    if (LATEX_FUNCTIONS.has(name))
        return mathRun(name, true);
    switch (name) {
        case 'frac':
        case 'dfrac':
        case 'tfrac': {
        ...
        case 'binom': {
            ...
            return ('<m:d><m:e><m:f><m:fPr><m:type m:val="noBar"/></m:fPr>' +
        ...
        case 'sqrt': {
            ...
            if (peek(p) === '[') {
                ...
                if (close === -1)
                    throw new LatexError('Missing matching ]');
                const sub = { src: p.src.slice(p.pos, close), pos: 0 };
        ...
        case 'underbrace':
            return ('<m:groupChr><m:groupChrPr><m:chr m:val="⏟"/><m:pos m:val="bot"/></m:groupChrPr>' +
                `<m:e>${parseGroup(p)}</m:e></m:groupChr>`);
        case 'overbrace':
            return ('<m:groupChr><m:groupChrPr><m:chr m:val="⏞"/><m:pos m:val="top"/></m:groupChrPr>' +
                `<m:e>${parseGroup(p)}</m:e></m:groupChr>`);
        case 'text':
        case 'mathrm':
        case 'operatorname':
        ...
        case 'left': {
            ...
            return ('<m:d><m:dPr>' +
                `<m:begChr m:val="${(0, xml_utils_1.escapeXmlAttr)(beg)}"/><m:endChr m:val="${(0, xml_utils_1.escapeXmlAttr)(end)}"/>` +
                `</m:dPr><m:e>${body}</m:e></m:d>`);
        }
        case 'begin': {
            const env = readBraceText(p);
            if (!(env in MATRIX_DELIMS))
                throw new LatexError(`Unsupported environment: \\begin{${env}}`);
