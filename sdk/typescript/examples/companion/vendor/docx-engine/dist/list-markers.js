...
exports.bulletMarkerScale = bulletMarkerScale;
exports.computeListMarkerInfos = computeListMarkerInfos;
exports.computeListMarkers = computeListMarkers;
...
function toChinese(value) {
    ...
    for (let i = 0; i < digits.length; i++) {
        ...
        if (d === 0) {
            if (!out.endsWith('零') && i < digits.length - 1)
                out += '零';
        }
        else {
...
const SUBSTITUTE_BULLET_SCALE = { '•': 1.25, '●': 1.35 };
function bulletMarkerScale(glyph) {
    return SUBSTITUTE_BULLET_SCALE[glyph] ?? 1;
}
function bulletInfo(text, level) {
    const font = level.font?.trim();
    if (!font || !(0, symbol_fonts_1.isSymbolFont)(font))
        return { text };
    return { text, symbolChar: (0, symbol_fonts_1.toSymbolPua)(level.lvlText), symbolFont: font };
}
/**
...
function computeListMarkerInfos(items, defs) {
    ...
    return items.map((item) => {
        ...
        if (level.numFmt === 'bullet') {
            ...
            const decoded = level.font ? (0, symbol_fonts_1.decodeSymbolText)(level.font, level.lvlText) : null;
            ...
            if (!glyph || /[-]/.test(glyph) || (decoded === null && (0, symbol_fonts_1.isSymbolFont)(level.font)))
