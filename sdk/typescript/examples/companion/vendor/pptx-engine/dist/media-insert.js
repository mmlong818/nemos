...
exports.solidPng = solidPng;
exports.addMedia = addMedia;
exports.addModel3d = addModel3d;
exports.model3dPartOf = model3dPartOf;
/**
...
function solidPng(w, h, rgb) {
    ...
    return Buffer.concat([
        ...
        pngChunk('IHDR', ihdr),
        pngChunk('IDAT', (0, node_zlib_1.deflateSync)(raw)),
...
function appendRels(opened, slide, rels) {
    ...
    let xml = opened.archive.readText(relsPath) ??
    ...
    for (const rel of rels) {
        ...
        xml = xml.replace('</Relationships>', `<Relationship Id="${rid}" Type="${rel.type}" Target="${(0, xml_utils_1.escapeXmlAttr)(rel.target)}"${mode}/></Relationships>`);
    }
...
function addMedia(opened, slideIndex, opts) {
    ...
    ensureDefaultContentType(opened, posterExt, posterExt === 'jpg' || posterExt === 'jpeg' ? 'image/jpeg' : 'image/png');
    ...
    const o = opts.offset;
    const xml = `<p:pic><p:nvPicPr><p:cNvPr id="${id}" name="${(0, xml_utils_1.escapeXmlAttr)(name)}"/>` +
    ...
    const r = (0, index_1.appendRawElements)(opened, slideIndex, [xml]);
...
function addModel3d(opened, slideIndex, opts) {
    ...
    const o = opts.offset;
    const xml = `<p:pic><p:nvPicPr><p:cNvPr id="${id}" name="${(0, xml_utils_1.escapeXmlAttr)(name)}" descr="${(0, xml_utils_1.escapeXmlAttr)(`aislides-3d:${modelPath}`)}"/>` +
