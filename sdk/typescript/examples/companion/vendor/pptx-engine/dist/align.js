...
exports.alignRects = alignRects;
exports.distributeRects = distributeRects;
// ── Internal helpers ────────────────────────────────────────────────────────
 ...
 */
function alignRects(rects, kind, containerRect) {
 ...
 * @returns     each element's new { x, y } (same order as rects)
 */
function distributeRects(rects, kind) {
    if (rects.length < 3)
    ...
    if (kind === 'horizontal') {
        ...
        const result = rects.map((r) => ({ x: r.x, y: r.y }));
