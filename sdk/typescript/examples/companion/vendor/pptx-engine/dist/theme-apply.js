...
exports.patchThemeXml = patchThemeXml;
exports.applyThemeToArchive = applyThemeToArchive;
exports.collectExplicitColors = collectExplicitColors;
exports.buildColorMap = buildColorMap;
exports.recolorXml = recolorXml;
exports.remapDeckColors = remapDeckColors;
...
const hex6 = (c) => c.replace(/^#/, '').slice(0, 6).toUpperCase();
...
function patchThemeXml(xml, spec) {
    ...
    for (const key of SCHEME_KEYS) {
        ...
        if (!color)
            continue;
        out = out.replace(new RegExp(`<a:${key}>[\\s\\S]*?</a:${key}>`), `<a:${key}><a:srgbClr val="${hex6(color)}"/></a:${key}>`);
    }
    out = out.replace(/(<a:clrScheme name=")[^"]*(")/, `$1${(0, xml_utils_1.escapeXmlAttr)(spec.name)}$2`);
    // Fonts: replace only the major/minor Latin typeface; East Asian / cs kept
    if (spec.majorFont) {
        out = out.replace(/(<a:majorFont>[\s\S]*?<a:latin[^>]*typeface=")[^"]*(")/, `$1${(0, xml_utils_1.escapeXmlAttr)(spec.majorFont)}$2`);
    }
    if (spec.minorFont) {
        out = out.replace(/(<a:minorFont>[\s\S]*?<a:latin[^>]*typeface=")[^"]*(")/, `$1${(0, xml_utils_1.escapeXmlAttr)(spec.minorFont)}$2`);
    }
    return out;
}
...
const isNeutral = ({ s, l }) => s < 0.15 || l > 0.95 || l < 0.07;
