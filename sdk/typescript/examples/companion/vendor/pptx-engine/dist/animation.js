...
exports.DEFAULT_MOTION_PATH = void 0;
exports.animClassOf = animClassOf;
exports.elementSpid = elementSpid;
exports.buildTimingXml = buildTimingXml;
exports.patchSlideTimingXml = patchSlideTimingXml;
exports.readSlideTimingXml = readSlideTimingXml;
exports.patchSlideTimingIncrementalXml = patchSlideTimingIncrementalXml;
exports.setSlideAnimations = setSlideAnimations;
exports.getSlideAnimations = getSlideAnimations;
/** Effect → class (entrance/emphasis/exit/motion path). */
function animClassOf(effect) {
    switch (effect) {
        case 'appear':
        case 'fade':
        case 'flyIn':
...
exports.DEFAULT_MOTION_PATH = 'M 0 0 L 0.25 0';
...
function motionAnimXml(gen, tgt, dur, path) {
    const p = path.trim() || exports.DEFAULT_MOTION_PATH;
    const pathAttr = /\bE$/.test(p) ? p : `${p} E`;
    return (`<p:animMotion origin="layout" path="${pathAttr}" pathEditMode="relative">` +
...
function effectBehaviorsXml(gen, a) {
    ...
    switch (a.effect) {
        ...
        case 'pulse':
            return (`<p:animScale><p:cBhvr><p:cTn id="${gen.next()}" dur="${Math.max(1, Math.round(dur / 2))}" autoRev="1" fill="hold"/>${target}</p:cBhvr>` +
                '<p:by x="106000" y="106000"/></p:animScale>');
        case 'spin':
            return (`<p:animRot by="21600000"><p:cBhvr><p:cTn id="${gen.next()}" dur="${dur}" fill="hold"/>${target}` +
                '<p:attrNameLst><p:attrName>r</p:attrName></p:attrNameLst></p:cBhvr></p:animRot>');
        case 'grow':
            return (`<p:animScale><p:cBhvr><p:cTn id="${gen.next()}" dur="${dur}" fill="hold"/>${target}</p:cBhvr>` +
        ...
        case 'fadeOut':
            return (`<p:animEffect transition="out" filter="fade"><p:cBhvr><p:cTn id="${gen.next()}" dur="${dur}"/>${target}</p:cBhvr></p:animEffect>` +
                hideAtEnd);
        case 'flyOut':
            return (moveAnimXml(gen, target, dur, 'ppt_x', '#ppt_x', '#ppt_x') +
                moveAnimXml(gen, target, dur, 'ppt_y', '#ppt_y', '1+#ppt_h/2') +
                hideAtEnd);
        case 'wipeOut':
            return (`<p:animEffect transition="out" filter="wipe(down)"><p:cBhvr><p:cTn id="${gen.next()}" dur="${dur}"/>${target}</p:cBhvr></p:animEffect>` +
                hideAtEnd);
        case 'shrink':
        ...
        case 'zoomOut':
            return (`<p:animScale><p:cBhvr><p:cTn id="${gen.next()}" dur="${dur}" fill="hold"/>${target}</p:cBhvr>` +
                ...
                hideAtEnd);
        case 'motionPath':
            return motionAnimXml(gen, target, dur, a.motionPath ?? exports.DEFAULT_MOTION_PATH);
    }
}
/**
...
function rebuildTimingPreservingXml(timing, anims) {
    ...
    if (anims.length === 0) {
        if (!seqCtnM)
            return rebuildBldLst(timing, anims);
    ...
    if (seqCtnM) {
        ...
        if (childClose < 0)
            return null;
        const seqXml = '<p:seq concurrent="1" nextAc="seek">' +
...
function readSlideTimingXml(bodySuffix) {
    ...
    while ((m = re.exec(timing)) !== null && m.index < seq.end) {
        ...
        let dm;
        while ((dm = durRe.exec(body)) !== null)
            durationMs = Math.max(durationMs, Number(dm[1]));
        // pulse with autoRev: actual duration = dur*2
        if (/\bautoRev="1"/.test(body))
            durationMs *= 2;
        ...
        if (effect === 'motionPath') {
            ...
            anim.motionPath = (path ?? exports.DEFAULT_MOTION_PATH).replace(/\s*E$/, '').trim();
        }
        out.push(anim);
    }
    return out;
...
function makeGenAfterMaxId(timing) {
    let maxId = 0;
    for (const m of timing.matchAll(/\bid="(\d+)"/g))
...
function appendBldPs(timing, items) {
    ...
    if (bldEnd >= 0)
        return timing.slice(0, bldEnd) + add + timing.slice(bldEnd);
    const tEnd = timing.lastIndexOf('</p:timing>');
    if (tEnd < 0)
        return timing;
...
function patchSlideTimingIncrementalXml(bodySuffix, anims) {
    ...
    if (anims.length > old.length && old.every((o, i) => animEq(o, anims[i]))) {
        ...
        for (const a of old)
            grpBase.set(a.spid, (grpBase.get(a.spid) ?? 0) + 1);
    ...
    if (anims.length === old.length) {
        ...
        if (o.trigger !== a.trigger || o.spid !== a.spid)
            return null;
        if ((o.paragraph ?? null) !== (a.paragraph ?? null))
            return null;
        const next = anims[i + 1];
        if (next && next.trigger !== 'onClick')
            return null;
