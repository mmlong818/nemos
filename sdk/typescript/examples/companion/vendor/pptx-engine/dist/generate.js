...
exports.patchTextElementXml = patchTextElementXml;
exports.patchParagraphPPrXml = patchParagraphPPrXml;
exports.patchElementPPr = patchElementPPr;
exports.rebuildTxBody = rebuildTxBody;
exports.generateParagraphXml = generateParagraphXml;
exports.patchElementFill = patchElementFill;
exports.patchElementStroke = patchElementStroke;
exports.patchPictureSrcRect = patchPictureSrcRect;
exports.patchSlideBackgroundXml = patchSlideBackgroundXml;
exports.patchSlideTransitionXml = patchSlideTransitionXml;
exports.readSlideTransitionXml = readSlideTransitionXml;
exports.patchSlideAdvanceTimeXml = patchSlideAdvanceTimeXml;
exports.readSlideAdvanceTimeXml = readSlideAdvanceTimeXml;
exports.patchSlideHiddenXml = patchSlideHiddenXml;
exports.readSlideHiddenXml = readSlideHiddenXml;
exports.generateXfrmXml = generateXfrmXml;
exports.patchElementXfrm = patchElementXfrm;
exports.patchBodyPrAutofit = patchBodyPrAutofit;
...
function patchTextElementXml(el, originalXml) {
    ...
    const aligned = runSpans.length === modelRuns.length &&
        runSpans.length > 0 &&
...
function buildPPrGroup(p, group) {
    switch (group) {
        ...
        case 'bullet': {
            ...
            let s = '';
            if (b.color)
                s += `<a:buClr><a:srgbClr val="${hex6(b.color)}"/></a:buClr>`;
            if (b.sizePct != null)
                s += `<a:buSzPct val="${Math.round(b.sizePct * 1000)}"/>`;
            if (b.font)
                s += `<a:buFont typeface="${(0, xml_utils_1.escapeXmlAttr)(b.font)}"/>`;
            s +=
                b.type === 'number'
                    ? `<a:buAutoNum type="${(0, xml_utils_1.escapeXmlAttr)(b.numType ?? 'arabicPeriod')}"/>`
                    : `<a:buChar char="${(0, xml_utils_1.escapeXmlAttr)(b.char ?? '•')}"/>`;
            return s;
        }
    }
}
...
function patchParagraphPPrXml(paraXml, p, which) {
    ...
    const setPPrAttr = (name, value) => {
        ...
        if (re.test(openTag))
            openTag = openTag.replace(re, ` ${name}="${(0, xml_utils_1.escapeXmlAttr)(value)}"`);
        else
            openTag = openTag.replace(/^<a:pPr/, `<a:pPr ${name}="${(0, xml_utils_1.escapeXmlAttr)(value)}"`);
    };
    if (which.align)
        setPPrAttr('algn', p.align ? ALIGN_MAP[p.align] : undefined);
    if (which.level)
        setPPrAttr('lvl', p.level ? String(p.level) : undefined);
    if (which.indents) {
...
function patchElementPPr(el, xml, which) {
    ...
    if (spans.length !== paras.length || !spans.length)
        return null;
    let out = '';
    let cursor = 0;
...
function patchRun(runXml, run) {
    ...
    out = out.replace(/(<a:t(?:\s[^>]*)?>)([\s\S]*?)(<\/a:t>)/, (_all, open, _text, close) => {
        return `${open}${(0, xml_utils_1.escapeXmlText)(run.text)}${close}`;
    });
...
function patchRunProps(runXml, run) {
    const attrPatch = (rprOpen) => {
        ...
        tag = setAttr(tag, 'sz', run.fontSize != null && !run.fontSizeImplicit
    ...
    if (hasRPr) {
        ...
        const color = run.color
            ...
            : '';
        const font = run.fontFamily && !run.fontImplicit && !run.latinFont && !run.eaFont
            ? `<a:latin typeface="${(0, xml_utils_1.escapeXmlAttr)(run.fontFamily)}"/><a:ea typeface="${(0, xml_utils_1.escapeXmlAttr)(run.fontFamily)}"/>`
            : '';
...
function hlinkXml(run) {
    if (!run.hyperlinkRId)
        return '';
    return (`<a:hlinkClick xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:id="${(0, xml_utils_1.escapeXmlAttr)(run.hyperlinkRId)}"` +
        (run.hyperlinkAction ? ` action="${(0, xml_utils_1.escapeXmlAttr)(run.hyperlinkAction)}"` : '') +
        (run.hyperlinkTooltip ? ` tooltip="${(0, xml_utils_1.escapeXmlAttr)(run.hyperlinkTooltip)}"` : '') +
        '/>');
}
...
function patchRunHlink(runXml, run) {
    ...
    if ((existing?.[1] ?? undefined) === run.hyperlinkRId)
        return runXml;
    // Strip the old one (self-closing or paired)
    runXml = runXml.replace(/<a:hlinkClick\b[^>]*\/>|<a:hlinkClick\b[^>]*>[\s\S]*?<\/a:hlinkClick>/, '');
    ...
    if (m)
        return runXml.slice(0, m.index) + hlink + runXml.slice(m.index);
...
function patchRunFont(runXml, family) {
    const esc = (0, xml_utils_1.escapeXmlAttr)(family);
    if (/<a:latin\b/.test(runXml)) {
        ...
        if (/<a:ea\b/.test(runXml)) {
            ...
            runXml = runXml.replace(/(<a:latin\b[^>]*\/>|<a:latin\b[^>]*>[\s\S]*?<\/a:latin>)/, `$1<a:ea typeface="${esc}"/>`);
        }
        return runXml;
    }
...
function patchRunColor(runXml, color) {
    ...
    if (/<a:solidFill>\s*<a:schemeClr\b/.test(runXml)) {
        return runXml.replace(/<a:solidFill>\s*<a:schemeClr\b[^>]*?(?:\/>|>[\s\S]*?<\/a:schemeClr>)\s*<\/a:solidFill>/, `<a:solidFill><a:srgbClr val="${hex}"/></a:solidFill>`);
    }
...
function buildRPrAttrs(run) {
    let s = '';
    if (run.fontSize != null && !run.fontSizeImplicit)
        s += ` sz="${Math.round(run.fontSize * 100)}"`;
    if (run.bold)
        s += ' b="1"';
    if (run.italic)
        s += ' i="1"';
    if (run.underline && !run.underlineImplicit)
        s += ` u="${(0, xml_utils_1.escapeXmlAttr)(run.underlineStyle ?? 'sng')}"`;
    if (run.strike)
        s += ` strike="${(0, xml_utils_1.escapeXmlAttr)(run.strikeStyle ?? 'sngStrike')}"`;
    if (run.letterSpacing)
        s += ` spc="${Math.round(run.letterSpacing * 100)}"`;
    if (run.baseline)
        s += ` baseline="${Math.round(run.baseline * 1000)}"`;
    return s;
}
...
function setAttr(tag, name, value, existingRe) {
    ...
    if (existingRe.test(tag)) {
        return tag.replace(existingRe, ` ${name}="${(0, xml_utils_1.escapeXmlAttr)(value)}"`);
    }
    // Inject after the tag name (after <a:rPr)
    return tag.replace(/^<a:rPr/, `<a:rPr ${name}="${(0, xml_utils_1.escapeXmlAttr)(value)}"`);
}
...
function generateParagraphXml(p) {
    ...
    const pPrAttrs = [];
    if (p.marL != null && want('marL'))
        pPrAttrs.push(`marL="${Math.round(p.marL)}"`);
    if (p.indent != null && want('indent'))
        pPrAttrs.push(`indent="${Math.round(p.indent)}"`);
    if (p.align && want('align'))
        pPrAttrs.push(`algn="${alignMap[p.align]}"`);
    if (p.level)
        pPrAttrs.push(`lvl="${p.level}"`);
    ...
    if (p.bullet && want('bullet')) {
        ...
        if (b.type === 'none')
            ...
            if (b.color)
                kids += `<a:buClr><a:srgbClr val="${hex6(b.color)}"/></a:buClr>`;
            if (b.sizePct != null)
                kids += `<a:buSzPct val="${Math.round(b.sizePct * 1000)}"/>`;
            if (b.font)
                kids += `<a:buFont typeface="${(0, xml_utils_1.escapeXmlAttr)(b.font)}"/>`;
            kids +=
                b.type === 'number'
                    ? `<a:buAutoNum type="${(0, xml_utils_1.escapeXmlAttr)(b.numType ?? 'arabicPeriod')}"/>`
                    : `<a:buChar char="${(0, xml_utils_1.escapeXmlAttr)(b.char ?? '•')}"/>`;
...
function generateRunXml(r) {
    ...
    const color = r.color && !r.colorInherited
    ...
    const font = r.latinFont || r.eaFont || r.csFont
        ? (r.latinFont ? `<a:latin typeface="${(0, xml_utils_1.escapeXmlAttr)(r.latinFont)}"/>` : '') +
            (r.eaFont ? `<a:ea typeface="${(0, xml_utils_1.escapeXmlAttr)(r.eaFont)}"/>` : '') +
            (r.csFont ? `<a:cs typeface="${(0, xml_utils_1.escapeXmlAttr)(r.csFont)}"/>` : '')
        : r.fontFamily && !r.fontImplicit
            ? `<a:latin typeface="${(0, xml_utils_1.escapeXmlAttr)(r.fontFamily)}"/><a:ea typeface="${(0, xml_utils_1.escapeXmlAttr)(r.fontFamily)}"/>`
            : '';
    ...
    if (r.field) {
        return (`<a:fld id="{${fieldGuid()}}" type="${(0, xml_utils_1.escapeXmlAttr)(r.field)}">` +
            `${rPr}<a:t>${(0, xml_utils_1.escapeXmlText)(r.text)}</a:t></a:fld>`);
    }
    return `<a:r>${rPr}<a:t>${(0, xml_utils_1.escapeXmlText)(r.text)}</a:t></a:r>`;
}
...
function topLevelChildren(xml, from, to) {
    ...
    while ((m = re.exec(xml)) !== null && m.index < to) {
        ...
        if (!closing && !selfClose) {
            ...
            depth--;
            if (depth === 0)
                out.push({ name: curName, start: curStart, end: re.lastIndex });
        }
...
function expandEmptySpPr(xml) {
    return xml.replace(/<p:spPr(\s[^>]*?)?\/>/, (_all, attrs) => `<p:spPr${attrs ?? ''}></p:spPr>`);
}
function gsLstXml(stops) {
    return `<a:gsLst>${stops
        .map((s) => `<a:gs pos="${Math.round(Math.max(0, Math.min(1, s.pos)) * 100000)}"><a:srgbClr val="${hex6(s.color)}"/></a:gs>`)
        .join('')}</a:gsLst>`;
}
const RADIAL_PATH_XML = '<a:path path="circle"><a:fillToRect l="50000" t="50000" r="50000" b="50000"/></a:path>';
function buildFillXml(fill) {
    if (typeof fill === 'object' && 'rawFillXml' in fill)
        return fill.rawFillXml;
...
function patchElementFill(originalXml, fill) {
    ...
    if (existing) {
        const fillXml = patchFillNodeXml(originalXml.slice(existing.start, existing.end), existing.name, fill);
    ...
    const anchor = children.find((c) => c.name === 'a:prstGeom' || c.name === 'a:custGeom') ??
...
function patchLnXml(lnXml, stroke) {
    ...
    const dashXml = stroke?.dash && stroke.dash !== 'solid'
        ? `<a:prstDash val="${(0, xml_utils_1.escapeXmlAttr)(stroke.dash)}"/>`
        : '';
    if (open[2] === '/')
        return `<a:ln${attrs}>${fillXml}${dashXml}</a:ln>`;
    const innerStart = open[0].length;
...
function patchElementStroke(originalXml, stroke) {
    ...
    const lnXml = stroke
        ? `<a:ln w="${Math.round(stroke.widthEmu)}"><a:solidFill>${srgbClrXml(stroke.color)}</a:solidFill>${stroke.dash && stroke.dash !== 'solid'
            ? `<a:prstDash val="${(0, xml_utils_1.escapeXmlAttr)(stroke.dash)}"/>`
            : ''}</a:ln>`
        : '<a:ln><a:noFill/></a:ln>';
    // Insert after fill / geometry / xfrm (OOXML order: xfrm → geom → fill → ln)
    const anchor = children.find((c) => FILL_TAGS.has(c.name)) ??
...
function patchPictureSrcRect(originalXml, srcRect) {
    ...
    const existing = /<a:srcRect[^/]*\/>|<a:srcRect[^>]*>[\s\S]*?<\/a:srcRect>/.exec(originalXml.slice(blipFillOpen.index, blipFillClose + '</p:blipFill>'.length));
    ...
    const to5 = (v) => Math.round(Math.max(0, Math.min(1, v)) * 100000);
...
function patchSlideBackgroundXml(bodyPrefix, color) {
    ...
    if (existing) {
        return (bodyPrefix.slice(0, existing.index) +
            bgXml +
 ...
 */
const MORPH_TRANSITION_XML = '<mc:AlternateContent xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006">' +
...
function transitionInsertPos(body) {
    ...
    if (clrMap)
        return clrMap.index + clrMap[0].length;
...
function patchAdvTmAttr(block, ms) {
    return block.replace(/<p:transition\b[^>]*>/g, (tag) => {
        ...
        if (ms == null)
            return cleaned;
        return cleaned.replace(/(\s*\/)?>$/, (_m, close) => ` advTm="${ms}"${close ?? ''}>`);
    });
}
/**
...
function patchSlideAdvanceTimeXml(bodySuffix, ms) {
    const m = AC_TRANSITION_RE.exec(bodySuffix) ?? TRANSITION_RE.exec(bodySuffix);
    if (m)
        return (bodySuffix.slice(0, m.index) +
...
function readSlideHiddenXml(bodyPrefix) {
    const m = SLD_OPEN_RE.exec(bodyPrefix);
...
function patchElementXfrm(el, originalXml) {
    ...
    if (open) {
        ...
        const attrs = (t.rot ? ` rot="${t.rot}"` : '') +
            (t.flipH ? ' flipH="1"' : '') +
            (t.flipV ? ' flipV="1"' : '');
        const o = t.offset;
        const replaced = `<${xfrmTag}${attrs}>` +
    ...
    if (spPr[3] === '/') {
        ...
        return (originalXml.slice(0, spPr.index) +
            openTag +
            generateXfrmXml(t) +
            `</${tag}>` +
...
function patchBodyPrAutofit(xml, fontScale, lnSpcReduction) {
    const attrs = (fontScale < 0.999 ? ` fontScale="${Math.round(fontScale * 100000)}"` : '') +
        (lnSpcReduction ? ` lnSpcReduction="${Math.round(lnSpcReduction * 100000)}"` : '');
    return xml.replace(/<a:normAutofit\b[^>]*?(?:\/>|>\s*<\/a:normAutofit>)/, `<a:normAutofit${attrs}/>`);
}
