...
exports.notesPathForSlide = notesPathForSlide;
exports.getSlideNotes = getSlideNotes;
...
function getSlideNotes(archive, slidePath) {
    ...
    if (!tx)
        return '';
    const paras = [...tx.matchAll(/<a:p>([\s\S]*?)<\/a:p>/g)].map((p) => [...p[1].matchAll(/<a:t>([\s\S]*?)<\/a:t>/g)].map((t) => unescapeXml(t[1])).join(''));
    // Drop trailing empty paragraphs (PowerPoint templates often carry an empty placeholder paragraph)
    while (paras.length && paras[paras.length - 1] === '')
        paras.pop();
    return paras.join('\n');
}
...
function buildNotesTxBody(text) {
    ...
    const paras = lines.every((l) => l === '')
        ...
        : lines
            .map((line) => line === ''
            ? '<a:p><a:endParaRPr lang="zh-CN"/></a:p>'
            : `<a:p><a:r><a:rPr lang="zh-CN" dirty="0"/><a:t>${(0, xml_utils_1.escapeXmlText)(line)}</a:t></a:r></a:p>`)
            .join('');
    return `<p:txBody><a:bodyPr/><a:lstStyle/>${paras}</p:txBody>`;
}
const NOTES_BODY_SP_OPEN = '<p:sp><p:nvSpPr><p:cNvPr id="3" name="Notes Placeholder"/>' +
...
function appendRelationship(archive, partPath, type, target) {
    ...
    let xml = archive.readText(relsPath) ??
        XMLDECL +
    ...
    xml = xml.replace('</Relationships>', `<Relationship Id="${rid}" Type="${type}" Target="${target}"/></Relationships>`);
...
function ensureNotesMaster(archive) {
    for (const path of archive.entries.keys()) {
        if (/^ppt\/notesMasters\/notesMaster\d+\.xml$/.test(path))
            return path;
    }
    ...
    if (theme)
        appendRelationship(archive, path, THEME_REL, `../${theme.slice(4)}`);
