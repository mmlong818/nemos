...
exports.buildChartSpaceXml = buildChartSpaceXml;
...
function strCacheXml(values, f) {
    return (`<c:strRef><c:f>${(0, xml_utils_1.escapeXmlText)(f)}</c:f><c:strCache><c:ptCount val="${values.length}"/>` +
        values.map((v, i) => (v === '' ? '' : `<c:pt idx="${i}"><c:v>${(0, xml_utils_1.escapeXmlText)(v)}</c:v></c:pt>`)).join('') +
        '</c:strCache></c:strRef>');
}
...
const DLBLS_XML = '<c:dLbls><c:showLegendKey val="0"/><c:showVal val="1"/><c:showCatName val="0"/>' +
...
function axTitleXml(text, vertical) {
    return ('<c:title><c:tx><c:rich>' +
        ...
        `<a:p><a:r><a:t>${(0, xml_utils_1.escapeXmlText)(text)}</a:t></a:r></a:p>` +
...
function buildChartSpaceXml(opts) {
    ...
    const title = opts.title
        ? '<c:title><c:tx><c:rich><a:bodyPr/><a:lstStyle/><a:p><a:r>' +
            `<a:t>${(0, xml_utils_1.escapeXmlText)(opts.title)}</a:t></a:r></a:p></c:rich></c:tx>` +
            '<c:overlay val="0"/></c:title><c:autoTitleDeleted val="0"/>'
        : '<c:autoTitleDeleted val="1"/>';
    const legendPos = opts.legendPos ?? 'b';
    const legend = legendPos === 'none'
        ? ''
...
function addChart(opened, slideIndex, opts) {
    ...
    if (!slide || !opts.categories.length || !opts.series.length)
        return null;
    ...
    const frameXml = `<p:graphicFrame><p:nvGraphicFramePr><p:cNvPr id="${id}" name="${(0, xml_utils_1.escapeXmlAttr)(name)}" descr="aislides-chart"/>` +
    ...
    const r = (0, index_1.appendRawElements)(opened, slideIndex, [frameXml]);
