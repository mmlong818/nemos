...
function newSectionId() {
    return `{${(0, node_crypto_1.randomUUID)().toUpperCase()}}`;
}
...
function readSlideEntries(opened) {
    ...
    for (const m of inner.matchAll(/<p:sldId\b[^>]*\/>/g)) {
        ...
        const path = rel ? (0, zip_1.resolveTarget)(PRES_PATH, rel.target) : undefined;
...
function getSections(opened) {
    ...
    for (const m of lst.matchAll(/<p14:section\b([^>]*?)(?:\/>|>([\s\S]*?)<\/p14:section>)/g)) {
        const attrs = m[1] ?? '';
        const inner = m[2] ?? '';
        const name = (0, notes_1.unescapeXml)(/\bname="([^"]*)"/.exec(attrs)?.[1] ?? '');
...
function setSections(opened, sections) {
    ...
    let next = pres.replace(/<p:ext\b[^>]*>\s*<p14:sectionLst[\s\S]*?<\/p14:sectionLst>\s*<\/p:ext>/, '');
    ...
    if (sections.length) {
        const secXml = sections
            ...
            const lst = ids.length
                ...
                : '<p14:sldIdLst/>';
            return `<p14:section name="${(0, xml_utils_1.escapeXmlAttr)(s.name)}" id="${s.id || newSectionId()}">${lst}</p14:section>`;
        })
            .join('');
...
function normalizeSections(sections, total) {
    if (!sections.length)
        return { lead: range(0, total), starts: [], sections: [] };
...
function addSection(opened, atSlideIndex, name) {
    ...
    if (atSlideIndex < 0 || atSlideIndex >= total)
        return null;
...
function moveSlide(opened, fromIndex, toIndex) {
    ...
    archive.entries.set(PRES_PATH, Buffer.from(pres.replace(m[0], `<p:sldIdLst>${tags.join('')}</p:sldIdLst>`), 'utf8'));
    // Sync deck.slides
...
function moveSection(opened, id, dir) {
    ...
    archive.entries.set(PRES_PATH, Buffer.from(pres.replace(m[0], `<p:sldIdLst>${newInner}</p:sldIdLst>`), 'utf8'));
