...
exports.LEGACY_NO_STYLE = void 0;
exports.resolveTableStyle = resolveTableStyle;
exports.cellStyleBorders = cellStyleBorders;
exports.cellPartStyle = cellPartStyle;
/**
...
exports.LEGACY_NO_STYLE = '{2D5ABB26-0587-4C30-8999-92F81FD0307D}';
...
const line = (color, width = 12700) => ({ fill: solid(color), width });
/**
...
function builtinStyle(family, accentName, theme) {
    const lt1 = (0, theme_1.resolveSchemeColor)('lt1', theme) ?? '#FFFFFF';
    const dk1 = (0, theme_1.resolveSchemeColor)('dk1', theme) ?? '#000000';
    const accent = accentName ? ((0, theme_1.resolveSchemeColor)(accentName, theme) ?? '#4472C4') : undefined;
    switch (family) {
        case 'themed1': {
            if (!accent)
                return { wholeTbl: { textColor: dk1 } }; // No Style, No Grid
        ...
        case 'dark2': {
            ...
            const header = accentName ? ((0, theme_1.resolveSchemeColor)(headerPair[accentName] ?? accentName, theme) ?? dk1) : dk1;
            const band = solid(tint(a, 0.4));
            return {
                wholeTbl: { fill: solid(tint(a, 0.2)), textColor: dk1 },
...
function resolveTableStyle(styleId, tableStylesXml, theme) {
    ...
    if (builtin)
        return builtinStyle(builtin.family, builtin.accent, theme);
    if (styleId === exports.LEGACY_NO_STYLE)
        return {};
    return undefined;
}
/**
...
function cellStyleBorders(def, flags, r, c, nRows, nCols) {
    const out = {};
    if (r < nRows - 1 && def.insideH)
        out.b = def.insideH;
    if (c < nCols - 1 && def.insideV)
        out.r = def.insideV;
    if (def.outer) {
        if (r === 0 && def.outer.t)
            out.t = def.outer.t;
        if (r === nRows - 1 && def.outer.b)
            out.b = def.outer.b;
        if (c === 0 && def.outer.l)
            out.l = def.outer.l;
        if (c === nCols - 1 && def.outer.r)
            out.r = def.outer.r;
    }
    if (flags.firstRow && r === 0 && nRows > 1 && def.firstRowBottom)
        out.b = def.firstRowBottom;
    if (flags.lastRow && r === nRows - 1 && nRows > 1 && def.lastRowTop)
        out.t = def.lastRowTop;
    return out;
}
// ── tableStyles.xml parsing ─────────────────────────────────────────
const tsParser = new fast_xml_parser_1.XMLParser({
...
function readColor(node, theme) {
    ...
    const srgb = (0, xml_utils_1.asXmlNode)(n['a:srgbClr'])['@_val'];
    if (srgb)
        return '#' + String(srgb).toUpperCase();
    if (n['a:schemeClr']) {
        const scheme = (0, xml_utils_1.asXmlNode)(n['a:schemeClr']);
        const base = (0, theme_1.resolveSchemeColor)(String(scheme['@_val']), theme);
        if (!base)
            return undefined;
        const t = (0, xml_utils_1.asXmlNode)(scheme['a:tint'])['@_val'];
        return t ? tint(base, parseInt(String(t), 10) / 100000) : base;
    }
    const prst = (0, xml_utils_1.asXmlNode)(n['a:prstClr'])['@_val'];
    if (prst === 'black')
        return '#000000';
    if (prst === 'white')
        return '#FFFFFF';
...
function readPart(part, theme) {
    if (!part || typeof part !== 'object')
        return undefined;
    const p = (0, xml_utils_1.asXmlNode)(part);
    const out = {};
    const fillColor = readColor((0, xml_utils_1.asXmlNode)((0, xml_utils_1.asXmlNode)(p['a:tcStyle'])['a:fill'])['a:solidFill'], theme);
    if (fillColor)
        out.fill = solid(fillColor);
    if (p['a:tcTxStyle']) {
        const tx = (0, xml_utils_1.asXmlNode)(p['a:tcTxStyle']);
        if (tx['@_b'] === 'on')
            out.bold = true;
...
function readInside(part, tag, theme) {
    const bdr = (0, xml_utils_1.asXmlNode)((0, xml_utils_1.asXmlNode)((0, xml_utils_1.asXmlNode)(part)['a:tcStyle'])['a:tcBdr']);
    const lnRaw = (0, xml_utils_1.asXmlNode)(bdr[tag])['a:ln'];
    if (!lnRaw)
        return undefined;
    const ln = (0, xml_utils_1.asXmlNode)(lnRaw);
...
function parseTableStylesXml(xml, styleId, theme) {
    let doc;
    try {
        doc = (0, xml_utils_1.asXmlNode)(tsParser.parse(xml));
    }
    catch {
        return undefined;
    }
    const list = (0, xml_utils_1.asXmlNode)(doc['a:tblStyleLst'])['a:tblStyle'] ?? [];
    const style = (0, xml_utils_1.xmlArray)(list).find((s) => s['@_styleId'] === styleId);
    if (!style)
        return undefined;
    const def = {};
...
function cellPartStyle(def, flags, r, c, nRows, nCols) {
    const merge = (base, over) => over ? { ...base, ...Object.fromEntries(Object.entries(over).filter(([, v]) => v !== undefined)) } : base;
    ...
    if (isLastCol)
        out = merge(out, def.lastCol);
    if (isFirstCol)
        out = merge(out, def.firstCol);
    if (isLastRow)
        out = merge(out, def.lastRow);
    if (isFirstRow)
        out = merge(out, def.firstRow);
    return out;
}
