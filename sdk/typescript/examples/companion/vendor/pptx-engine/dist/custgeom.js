...
exports.evalGuides = evalGuides;
exports.parseCustGeom = parseCustGeom;
...
function parseCustGeom(shapeXml, shapeW, shapeH) {
    ...
    while ((m = TAG_RE.exec(xml))) {
        ...
        switch (name) {
            ...
            case 'a:gd': {
                if (!inGuides)
                    break; // gd elsewhere (e.g. cxnLst) does not participate
            ...
            case 'a:cubicBezTo':
                if (cur)
                    pending = { c: CMD_LETTER[name], need: CMD_PT_COUNT[name], pts: [] };
                break;
            case 'a:pt': {
                if (!cur || !pending)
                    break;
    ...
    paths.forEach((p, i) => {
        if (!p.w)
            fw = Math.max(fw, maxCoord(absList[i], 0));
        if (!p.h)
            fh = Math.max(fh, maxCoord(absList[i], 1));
    });
    ...
    paths.forEach((p, i) => {
        ...
        if (fillNone && strokeNone)
            return;
        if (fillNone)
            buckets.strokePath.push(d);
        else if (strokeNone)
            buckets.fillPath.push(d);
        else
            buckets.path.push(d);
    });
    const out = {};
    if (buckets.path.length)
        out.path = buckets.path.join(' ');
    if (buckets.fillPath.length)
        out.fillPath = buckets.fillPath.join(' ');
    if (buckets.strokePath.length)
        out.strokePath = buckets.strokePath.join(' ');
...
function maxCoord(cmds, axis) {
    let mx = 0;
    for (const c of cmds)
        for (let i = axis; i < c.xy.length; i += 2)
            mx = Math.max(mx, c.xy[i]);
    return mx;
}
