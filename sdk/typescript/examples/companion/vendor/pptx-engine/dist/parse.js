...
exports.EMU_PER_PT = void 0;
exports.parseSlide = parseSlide;
exports.sliceGroupChildXmls = sliceGroupChildXmls;
...
const table_grid_1 = require("./table-grid");
const theme_1 = require("./theme");
const color_1 = require("./color");
const placeholder_1 = require("./placeholder");
const chart_1 = require("./chart");
const custgeom_1 = require("./custgeom");
...
function parseSlide(input) {
    ...
    const scan = (0, scan_1.scanSlide)(slideXml);
    ...
    const background = parseBackground(slideXml, ctx) ??
...
function parseSpShape(node, anchor, ctx, rawXml) {
    ...
    if (ph && !spPr['a:xfrm']) {
        const inherited = (0, placeholder_1.resolvePlaceholderTransform)(ctx.layoutPlaceholders, ctx.masterPlaceholders, phType, phIdx);
        if (inherited)
            transform = inherited;
    }
    ...
    const customGeometry = spPr['a:custGeom'] != null
        ? (0, custgeom_1.parseCustGeom)(rawXml || anchor.originalXml, transform.offset.cx, transform.offset.cy)
        : undefined;
    ...
    const phChain = ph
        ? (0, placeholder_1.placeholderStyleChain)(ctx.layoutPlaceholders, ctx.masterPlaceholders, ctx.masterTextStyles, phType, phIdx)
        : [];
    ...
    if (style && typeof style === 'object') {
        if (fill === undefined) {
            ...
            if (idx > 0) {
                ...
                const tpl = idx > 1000 ? ctx.theme?.bgFillStyles?.[idx - 1001] : ctx.theme?.fillStyles?.[idx - 1];
        ...
        if (stroke === undefined)
            stroke = styleRefStroke(node, ctx);
        if (!shadow && !glow) {
...
function styleRefStroke(node, ctx) {
    ...
    return ((tpl ? parseStroke(tpl, { ...ctx, phClr }) : undefined) ??
...
function parseConnector(node, anchor, ctx) {
    ...
    const fallback = (refStroke?.fill.type === 'solid' ? refStroke.fill.color : undefined) ??
    ...
    const stroke = explicitStroke === null
        ? undefined
        : (explicitStroke ??
            refStroke ??
            { fill: { type: 'solid', color: fallback }, width: 12700 });
    ...
    const cxnRef = (n) => n?.['@_id'] != null ? { id: parseInt(n['@_id'], 10), idx: intOr(n['@_idx'], 0) } : undefined;
...
function parseAvLst(avLst) {
    ...
    for (const gd of list) {
        ...
        if (name && m)
            out[name] = parseInt(m[1], 10);
    }
    return Object.keys(out).length ? out : undefined;
}
// ── p:grpSp (group) ────────────────────────────────────────
const GROUP_CHILD_TAGS = ['p:sp', 'p:pic', 'p:grpSp', 'p:graphicFrame', 'p:cxnSp'];
function parseGroup(node, anchor, ctx, rawXml) {
    ...
    for (const tag of GROUP_CHILD_TAGS) {
        ...
        list.forEach((child, i) => {
            ...
            if (el)
                ordered.push({ el, start: slice?.start ?? Number.MAX_SAFE_INTEGER });
        });
    }
    ...
    while ((m = GROUP_TAG_RE.exec(xml))) {
        ...
        if (closing) {
            ...
            if (depth === 1 && tags.has(name))
                out.push({ name, xml: tag, start: m.index });
        }
        else {
...
function parsePicture(node, anchor, ctx) {
    ...
    const opacity = alphaAmt != null ? Math.max(0, Math.min(1, parseInt(alphaAmt, 10) / 100000)) : undefined;
    ...
    let media;
    if (avNode !== undefined) {
        const kind = nvPr?.['a:videoFile'] !== undefined ? 'video' : 'audio';
...
function parseSrcRect(sr) {
    if (!sr || typeof sr !== 'object')
        return undefined;
    const f = (k) => intOr(sr[`@_${k}`], 0) / 100000;
    const rect = { l: f('l'), t: f('t'), r: f('r'), b: f('b') };
    if (!rect.l && !rect.t && !rect.r && !rect.b)
        return undefined;
    return rect;
}
...
function graphicFramePassthrough(node, anchor, ctx) {
    ...
    if (uri.includes('/chart')) {
        ...
        const model = chartXml ? (0, chart_1.parseChartXml)(chartXml, ctx.theme) : null;
        if (model) {
            const cNvPr = node['p:nvGraphicFramePr']?.['p:cNvPr'];
            const descr = cNvPr?.['@_descr'] || undefined;
            return {
                id: uid('chart'),
                type: 'chart',
                anchor,
    ...
    if (uri.includes('/table'))
        ...
        kind = 'chart';
    else if (uri.includes('/diagram') || uri.includes('SmartArt'))
        kind = 'smartart';
    else if (uri.includes('/ole'))
        kind = 'ole';
    ...
    if (kind === 'ole') {
        ...
        if (pic)
            el.previewPicture = parsePicture(pic, anchor, ctx);
    }
    return el;
}
/**
...
function parseTable(node, tbl, anchor, ctx) {
    ...
    if (!colWidths.length || !trs.length)
        return null;
    ...
    const styleDef = (0, table_style_1.resolveTableStyle)(styleId, ctx.tableStyles, ctx.theme);
    const flags = {
    ...
    const rows = trs.map((tr, r) => {
        ...
        const gridCols = (0, table_grid_1.tableRowGridCols)(tcs.map((tc) => ({
            gridSpan: tc['@_gridSpan'] ? parseInt(tc['@_gridSpan'], 10) || 1 : 1,
            merged: tc['@_hMerge'] === '1' || tc['@_vMerge'] === '1',
        })));
        return tcs.map((tc, i) => {
            const c = gridCols[i];
            const part = styleDef ? (0, table_style_1.cellPartStyle)(styleDef, flags, r, c, nRows, nCols) : undefined;
            const inside = styleDef ? (0, table_style_1.cellStyleBorders)(styleDef, flags, r, c, nRows, nCols) : undefined;
...
function parseTableCell(tc, ctx, part, inside) {
    ...
    if (tc['a:txBody']) {
        ...
        const styleChain = part && (part.bold !== undefined || part.textColor)
            ? [
                {
                    levels: [
                        {
        ...
        if (tcPr['@_anchor'])
            text.anchor = anchorMap[tcPr['@_anchor']];
        text.insets = {
...
function parseFill(spPr, ctx) {
    ...
    if (grad)
        return parseGradFill(grad, ctx);
    const blip = spPr['a:blipFill'];
    if (blip) {
        const embedId = blipEmbedId(blip['a:blip']);
        const mediaRef = (embedId && ctx.mediaRels?.get(embedId)) || '';
        if (mediaRef)
            return { type: 'image', mediaRef, mode: 'a:tile' in blip ? 'tile' : 'stretch' };
    }
...
function resolveColorNode(node, ctx) {
    return (0, color_1.resolveColorNode)(node, ctx.theme, ctx.phClr);
}
// ── Text ─────────────────────────────────────────────────────────────
function parseTextBody(txBody, ctx, phChain = []) {
    ...
    const ownStyle = (0, placeholder_1.parseLstStyleLevels)(txBody['a:lstStyle'], ctx.theme);
    const chain = [ownStyle, ...phChain];
    const paragraphs = paras.map((p) => parseParagraph(p, ctx, chain));
    let autofit = 'none';
    if ('a:normAutofit' in bodyPr)
        autofit = 'shrink';
    else if ('a:spAutoFit' in bodyPr)
        autofit = 'resize';
    ...
    const vert = vertRaw === 'eaVert' || vertRaw === 'vert' || vertRaw === 'vert270' || vertRaw === 'wordArtVert'
        ? vertRaw
        : undefined;
    return {
        paragraphs,
...
function parseParagraph(p, ctx, chain = []) {
    ...
    const dflt = (0, placeholder_1.mergeTextStyleChain)(chain, level ?? 0);
    ...
    if (pPr['a:buNone'] !== undefined)
        ...
        bullet = { type: 'number' };
        if (pPr['a:buAutoNum']['@_type'])
            bullet.numType = String(pPr['a:buAutoNum']['@_type']);
    }
    if (bullet && bullet.type !== 'none') {
        ...
        if (pPr['a:buFont']?.['@_typeface'])
            bullet.font = String(pPr['a:buFont']['@_typeface']);
        if (pPr['a:buSzPct']?.['@_val']) {
            const v = parseInt(pPr['a:buSzPct']['@_val'], 10);
            if (Number.isFinite(v))
                bullet.sizePct = v / 1000;
        }
    }
...
function parseRun(r, ctx, dflt) {
    ...
    const color = (fill ? resolveColorNode(fill, ctx) : undefined) ??
    ...
    const latin = (0, theme_1.resolveFontRef)(rPr['a:latin']?.['@_typeface'], ctx.theme) ?? dflt?.latinFont;
    const ea = (0, theme_1.resolveFontRef)(rPr['a:ea']?.['@_typeface'], ctx.theme) ?? dflt?.eaFont;
    const cs = (0, theme_1.resolveFontRef)(rPr['a:cs']?.['@_typeface'], ctx.theme) ?? dflt?.csFont;
    // Pick the bucket by script: complex script → a:cs, CJK → a:ea, otherwise → a:latin; fall back through buckets when missing
    const fontFamily = (CS_RE.test(text) ? (cs ?? latin ?? ea) : CJK_RE.test(text) ? (ea ?? latin) : (latin ?? ea)) ??
        ctx.theme?.minorFont;
    const bAttr = rPr['@_b'];
    const iAttr = rPr['@_i'];
...
function parseDecorations(xml, ctx, opts = {}) {
    ...
    scan.elements.forEach((sp, idx) => {
        ...
        const el = parseShapeFragment(sp, fragXml, anchor, ctx);
        if (!el || el.type === 'passthrough')
            return;
        const ph = el.placeholder;
        if (ph !== undefined) {
            if (!opts.hfTypes?.has(ph))
                return;
        }
        ...
        if (opts.slideNum != null)
            substituteSlideNum(el, opts.slideNum);
        out.push(el);
    });
    return out;
}
