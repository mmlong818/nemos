...
> 状态基线：2026-08-06。本文是 Agent Runtime 的当前实现与验收依据。

## 目标

把小丑鱼从“带记忆的角色聊天与分散能力”改造成能够可靠执行任务、恢复运行并交付真实结果的本地 Agent。

运行时遵循以下工程约束：

- 运行前统一组装上下文，技能和工具按需加载；
- 高影响动作经过统一用户动作网关；
- 插件、Skill、MCP 和 Agent App 使用明确契约；
- 模型、协议、工具、会话和记忆相互解耦；
- 取消、超时、并发、预算、输出和上下文具有硬边界；
- 多 Agent 只用于边界清楚、可独立验证的子任务。
...
- [`run-store.ts`](../../../src/agent/run-store.ts) 以追加式 JSONL 事件日志保存 run、round、tool call、tool result、停止原因、上下文接力与检查点，并从事件重建状态。启动时把未终结运行标成 interrupted；只读或尚未执行的安全步骤自动续跑，副作用状态不明确时停下人工核对。日志默认只保留工具摘要，并清理常见密钥和令牌。
...
小丑鱼不允许模型直接触达服务内部函数。工具调用先经过注册表和参数校验；未注册工具、未知字段和越权路径明确失败，不做模糊猜测。
...
- [x] 保存 session、run、round、tool call、tool result、stop reason 和上下文接力事件。`runId` 唯一标识一次执行，`sessionId` 可关联同一长期对话中的多次执行。
