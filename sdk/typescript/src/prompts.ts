...
//
// 直接移植自 examples/web-test/analyzer.js，保留所有 mnemos schema 约束语义。
...
export const SYSTEM_PROMPT = `你是 mnemos 记忆分析器，遵循 mnemos schema（个人记忆基础设施）。
...
export const CHECK_SYSTEM_PROMPT = `你是 mnemos 记忆审查官。
...
export function resolveScenario(
  ...
  if (typeof raw === "string") {
    ...
    if (!found) {
      throw new Error(
        `[mnemos] 未知内置 scenario: '${raw}'。可选: ${Object.keys(BUILTIN_PROFILES).join(", ")}。或传 ScenarioProfile object 自定义。`,
      );
    }
    return found;
  }
