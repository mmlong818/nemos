...
export async function runReflect(
  ...
  // RFC 0007 §2.3：词法粗筛——先用便宜的字符 bigram Jaccard 把"根本不相干"的 anchor 滤掉，
  // 只把"像在说同一件事"的少数候选送 LLM（判矛盾 / 避免重复），省 prompt、可扩展。
  // 阈值偏低（prefilter 默认 0.08），宁可多放进 LLM 也别误筛用词不同的矛盾。
  // 再过 capAnchors 兜大集合上限。allAnchor 为空时 relevant 也为空，行为不变。
  const relevant = prefilterCandidates(episodicText, allAnchor).map((a) => {
    const { score, ...m } = a;
    void score;
    return m as Memory;
  });
  const anchor = capAnchors(relevant, episodicText);
