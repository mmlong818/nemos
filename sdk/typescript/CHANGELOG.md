...
- The left navigation rail now draws its icons from the app's own icon set instead of Unicode geometric characters. It was the one place that bypassed the icon system: the nine characters came from three unrelated shape families, one of them was the macOS Command key symbol standing in for tools and connections, three differed only by fill pattern, and none of them exists in the declared font stack, so every icon was supplied by an undeclared fallback font and varied in weight, alignment and machine. The rail renders inline SVG server-side, which also keeps it correct on the one page that never loaded the icon script.
- Added outbound proxy support for the model calls, which previously had none: in a network that requires a proxy the app's requests simply timed out and scheduled tasks failed quietly. It is off by default and installs nothing when no proxy resolves, so anyone without a proxy sees no change; `off` ignores proxy environment variables outright. Loopback and private addresses always bypass the proxy, including the host of the saved model endpoint, so a local model server keeps working. An explicit address may not embed credentials because persisting one would need DPAPI and therefore could not work off Windows; a proxy that requires a login is configured through the environment instead and is never persisted. Scope is deliberately narrow and documented: `fetch`-based outbound calls only, so web reading, which pins its connection to the resolved address over `node:https`, is neither proxied nor weakened.
...
- A pre-v4 connection file has its checks bound once and is then rewritten as v4 immediately. Previously the binding ran on every start, each time minting a fresh revision and re-blessing the same stale checks, so the revision guard could never detect a mismatch.
