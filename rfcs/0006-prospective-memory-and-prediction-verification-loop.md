...
Nemos 现有 5 层全部是**回溯性**的——episodic（经历）、semantic（事实）、procedural（已成型的习惯）都记录已发生或已稳定的内容。系统能回答"我过去做过什么"，但无法回答"面对一个新情境，我大概会怎么应对"。
...
- Nemos RFC 0004（reflect / FSRS / surprise）：闭环复用的现有零件。
