---
rfc_number: 0001
title: mnemos Design Principles
authors:
...
mnemos 的核心设计原则。本 RFC 作为 founding document，任何后续 RFC 都必须与本文档兼容；与本文档冲突的 RFC 必须显式声明并经 60 天 governance 流程。

# Motivation

mnemos 来自 75 份研究 + ECC v2 v0 dogfood 验证。本 RFC 把那些研究产出固化为可被后续工作引用的原则集合。没有 founding document，后续 RFC 会重复辩论已有共识，浪费时间且可能漂移。

# Detailed Design

## 原则 1：AI 是仆人，不是代理（servant not agent）

**陈述**：mnemos 是 AI 应用的工具。所有 AI 推断的内容必须可追溯为"AI 推断"，不能伪装成用户陈述。
...
**陈述**：mnemos 不直接服务终端用户。客户是 AI 应用。查询完全由 AI 应用触发，按 AI 集成模式优化（hot-path 单查 < 100ms / session-start bulk < 2s / reflect 30s 可接受）。
...
## 原则 12：完全开源 + Apache-2.0

**陈述**：mnemos spec + 引用实现 + 默认 SDK 永远 Apache-2.0。商业衍生与 OSS 项目治理分离，不允许商业版本反向蚕食 OSS。
...
A：不排除。但商业 SaaS 是 OSS 项目之外的衍生，OSS 部分永远 Apache-2.0。
