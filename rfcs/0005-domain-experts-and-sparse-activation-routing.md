...
| **L2 邻接领域** | 路由器输出的 2~3 个次相关领域（centroid top-k 相似 / LLM 选）；`domain_affinity` 当前只供 merge，不参与 L2 路由 | top-k routed | 中 |
...
> **实现现状（2026-06-22）**：四级激活已全部接入检索——L0/L1/L2 由 `rerankByActivation` 加权重排（软隔离，降权不剔除）；**L3 跨域联想已沿 cross-memory 边接入**（命中领域的种子扩散一跳，限额 `l3SpreadLimit`，扩散件遵守失效/敏感/cold 可见性）。领域生命周期 birth/recompute/split/merge/sleep 均落地；**`domain_affinity` 已由 reflect 离线从跨域 related 边累积（「共边统计」）**，`mergeDomains` 第二道闸放宽为「成员交叠 OR 跨域连接紧密度」，单归属下也能合并。**仍待办**：① 候选扫描仍在排序层软重排、未在存储层按 `memory_domain` 剪枝（硬 MOE 的读放大收益，O(scope)→O(L1∪L2∪L0)）；② HybridRouter（质心+路由表硬触发）未实现；③ §7 第 3 道逃生阀「周期性全局重扫」未实现。均属优化项，受低置信回退全局保护，可安全后置。
