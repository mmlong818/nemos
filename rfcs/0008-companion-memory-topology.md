...
- **过滤已失效**：当前 `SearchOptions` 不按 `belief_state` 过滤（字段仅有索引意图，无 active 默认）。需补：检索默认 `belief_state='active'` 且排除 `invalid_at<=now` / `expired_at` 命中。详见 §7。
...
1. **检索默认失效过滤**（`SearchOptions`）：默认 `belief_state='active'` 且排除 `invalid_at<=now`/`expired_at` 非空；新增 `includeInvalidated?: boolean` 逃生阀。**与 RFC 0007 Step 3/4 重叠，建议并入 0007 实施**。
