...
| **文件** | 打开 Word、PowerPoint、Excel、PDF、TXT 或 Markdown | 原格式工作副本、Word 正文编辑、PPT 逐页编辑、Excel 单元格与公式、版本记录和安全写回 |
...
- Word 可直接修改正文并写入真实 DOCX 工作副本；既有段落格式、图片、页眉和页脚继续保留；
- PowerPoint 提供页面缩略列表和逐页文字编辑，写回时保留现有文本框、图片和页面布局；
- Excel 提供工作表标签、可编辑单元格、公式输入和确定性的数值概览，写回时保留既有单元格样式；
...
- 能力产物严格区分已生成、已校验、已核验和已确认；演示文稿会用 Edge 对关键页真实渲染截图并检查空白与画面层次，未执行真实复核时不会标为已核验；
