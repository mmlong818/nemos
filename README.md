...
“开发项目”使用独立工作台：选择目录后用普通语言描述结果，小丑鱼会查看项目、修改代码并运行检查。缺少依赖时，可按项目锁文件安装到项目目录或 Python 虚拟环境；不会进行全局安装，也不会执行模型临时编造的安装命令。修改受授权目录限制，项目在执行期间发生变化时会停止覆盖。
...
详细证据见[产品能力验收记录](docs/product-capability-acceptance-2026-08-13.md)。记忆内核测试由 `nemos-memory` 仓库独立维护，不在上述 369 项中重复计算。
