...
- Word 按段落修改：只重写你改过的段落，其余段落、表格、图片、图表、页眉页脚、批注和修订都保持原字节，改动段落自己的加粗、字号、颜色也保留。结果**另存为新文件**，打开的那个文件不被改动。已用真实 Microsoft Word 在禁止修复的模式下逐份验证（见 `sdk/typescript/examples/companion/docs/reviews/2026-08-10-docx-fidelity-with-word.md`）；
