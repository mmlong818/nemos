...
- 每个文件都标注真实能力：TXT 和 Markdown 是**可编辑**，Word、PowerPoint、Excel 和 PDF 是**仅查看**；
- Word、PowerPoint 和 Excel 的文字可以提取和修改，但只会**另存为新文件**，打开的那个文件不被改动；界面同时说明会损失什么——被改动段落的行内格式会合并，页眉页脚、批注、修订、图表和母版不在范围内；
