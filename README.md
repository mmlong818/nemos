...
| 已接入 | 尚未接入或不承诺 |
| --- | --- |
| 本机助理、目标、事项、任务、技能、万神殿、记忆、文件、成果和自动化 | 官方在线技能市场与第三方 Bot 账号同步 |
| 动态、点子与帮我盯着（联网部分需要可用的搜索服务，当前为智谱联网搜索） | 不配置联网搜索时的新消息与盯梢；此时动态只根据目标和事项写提醒与建议 |
| 可交互构件：沙箱内运行、本机保存状态、交付前浏览器自测、钉到总览 | 构件联网或调用应用接口；构件约定为单文件、不联网 |
| 免打扰时段、想听 / 别提、Windows 开机在托盘自动启动 | 应用未运行时的提醒；关机期间错过的提醒不补发 |
