...
“资料”页同时集中展示四类平台状态：GitHub、浏览器、邮箱和日历连接器是否真实可用；Node.js、Git 和 Python 等开发环境是否就绪；研究、办公、开发、运营和财务能力包的交付检查；以及已安装扩展的启用、停用和卸载。未安装的连接器只显示为未安装，不会冒充已接通；用户可以直接导入连接器清单，安装前会检查本机执行、沙箱和新增权限。扩展升级一旦扩大文件、网络、工具或模型权限，仍需重新确认。
...
- TypeScript 构建通过；
- 351 项自动化测试全部通过；记忆内核测试在独立的 `nemos-memory` 仓库维护，不在本仓库重复计算；
