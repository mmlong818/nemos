...
| 入口 | 用户操作 | 当前实现 |
|---|---|---|
| **聊天** | 提问、上传图片或文件、发送语音、继续历史对话 | 小丑鱼主入口、可选陪伴与学习角色、专家组协作、结果回到原对话 |
...
- **小丑鱼**负责日常问答、调用能力、组织专家和交付结果。

菲菲、林老师等陪伴与学习角色仍然保留，但由用户需要时自行添加，不再占据新用户首页。

首次打开角色对话时，页面会先简要说明这个角色能帮助什么、适合解决哪些问题，不需要用户先试探角色定位。

功能型专家不再作为一排一对一联系人出现，而是集中在“小丑鱼专家组”。专家不是首轮选定后永久固定：系统每轮都按当前问题判断，连续追问优先延续上一轮专家，主题变化时重新邀请，也可以通过 `@专家` 明确点名；小丑鱼负责最后统筹。聊天转交能力时，会同时保留用户原文和提炼后的任务上下文，而不是只传递最后一句话。
...
小丑鱼会按服务商预设分配模型：支持独立日常对话模型的服务会自动分流，专家、能力、文件生成和复杂任务使用这里配置的任务模型；没有独立分流时，两类任务使用同一个模型。也可以在单个对话的设置中手动指定模型。
