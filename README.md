...
“资料”页同时集中展示四类平台状态：GitHub、浏览器、邮箱和日历连接器是否真实可用；Node.js、Git 和 Python 等开发环境是否就绪；研究、办公、开发、运营和财务能力包的交付检查；以及已安装扩展的启用、停用和卸载。未安装的连接器只显示为未安装，不会冒充已接通。扩展升级一旦扩大文件、网络、工具或模型权限，仍需重新确认。

“运行”页顶部提供统一审阅队列，只收拢待确认操作、无法自动判断是否产生副作用的任务、失败运行和待审开发修改。普通后台任务继续并行执行，不会不断打断用户。
