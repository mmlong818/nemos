...
任务在后台持续执行，保留检查点、取消、失败原因、重试入口和最终产物。一个能力的结果可以继续交给另一个能力，仍沿用同一任务脉络。

“开发项目”使用独立工作台：查看项目文件、逐文件审阅修改、运行检查、选择性写入和回滚。修改受授权目录限制，项目在执行期间发生变化时会停止覆盖。
...
在应用中打开“模型连接”，选择服务商与模型并填写 API Key。连接验证通过后才会保存。
...
- 构建、类型检查和 **359 项自动化测试**全部通过；
- **20/20** 份脱敏 DOCX 通过结构往返，并由本机 Microsoft Word 打开；
- **10/10** 类小项目通过检查、修改提案、选择性写入和回滚；
- 全新数据环境完成 **20 轮真实使用检查**，20 轮通过、0 个未修问题；
- 离线依赖审计未发现已知漏洞，敏感信息扫描未发现密钥。

详细证据见[产品能力验收记录](docs/product-capability-acceptance-2026-08-13.md)。记忆内核测试由 `nemos-memory` 仓库独立维护，不在上述 359 项中重复计算。
