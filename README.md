...
| 部分 | 是什么 | 授权 |
|---|---|---|
| **Nemos TypeScript 接入层**<br>`sdk/typescript/` | 通过 `@nemos/sdk` 使用独立维护的记忆内核，并在本仓库提供 Agent 运行时与小丑鱼应用 | [PolyForm Noncommercial 1.0.0](LICENSE)，商业用途需另行授权 |
...
“开发项目”会进入独立项目工作台：左侧查看项目文件树和逐文件修改，中央对照修改前后内容，右侧核对环境、测试和构建结果。修改默认在隔离副本中执行，用户可以只写入选中的文件，项目发生并发变化时停止覆盖，并可在写入后恢复。
...
- 上传的文字文档在本机转成标题、段落、列表、表格、引用、代码和来源位置组成的结构化副本；Markdown 作为交换表示，TXT 和 Markdown 本身直接编辑；
...
“资料”页同时集中展示六类平台状态：本地文件、浏览器、GitHub、邮箱、日历和企业文档连接器是否真实可用；Node.js、Git、Python、Go、Rust、Java 和 .NET 等开发环境是否就绪；研究、办公、开发、运营和财务能力包的交付检查；以及已安装扩展的启用、更新、恢复上一版、停用和卸载。未安装的连接器只显示为未安装，不会冒充已接通；用户可以直接导入连接器清单，安装前会检查本机执行、沙箱和新增权限。扩展升级一旦扩大文件、网络、工具或模型权限，仍需重新确认。

“运行”页顶部提供统一审阅队列，只收拢待确认操作、无法自动判断是否产生副作用的任务、失败运行和待审开发修改。普通后台任务继续并行执行，不会不断打断用户。产品真实检查也会按人物、场景、路径、观察、问题与证据留存在这里，修复后仍可追溯。
...
- TypeScript 构建通过；
- 359 项自动化测试全部通过；记忆内核测试在独立的 `nemos-memory` 仓库维护，不在本仓库重复计算；
- 20 份脱敏日常 DOCX 已通过结构回读并由本机 Microsoft Word 逐份打开；10 类小项目已通过检查、修改提案、选择性写入与回滚回归；
- 首页、能力、文件、工作、记忆和模型连接已用全新本机数据目录重新截图核对；
- 能力产物严格区分已生成、已校验、已核验和已确认；演示文稿会用本机 Chromium 浏览器对关键页真实渲染截图并检查空白与画面层次，未执行真实复核时不会标为已核验；
- 生成能力升级会重新执行准入矩阵并保存合同指纹，触发边界冲突或夹具缺失时拒绝安装；外部安装能力也必须包含可执行步骤和结果约定；能力内容被异常改动时会停止执行，并允许从版本历史恢复；
