...
> 端到端 PoC：在浏览器里完整跑一遍 mnemos 的「上传 → 分析 → 5 层分级存储 → 检查」流程。
>
...
- ✅ mnemos schema（5 层 + 三维元数据 + source.authoritative）在真实数据上可工作
...
python -m http.server 8000 -d E:/CC/code/mnemos/examples/web-test/
...
- 数据全在浏览器 IndexedDB（DB 名 `mnemos-poc`）
- 想完全清除：浏览器 DevTools → Application → IndexedDB → 删除 `mnemos-poc`
