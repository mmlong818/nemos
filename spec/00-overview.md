...
**Nemos 是多人格 AI 陪伴 App 的记忆系统——一个可独立于该 App 运行的多租户记忆层**。陪伴 App 是它的旗舰应用与 forcing function，底层记忆系统设计上可被其他 AI 应用复用。
...
| 不是 | 理由 |
|---|---|
| 不是 enterprise B2B 平台 | 定位是 B2C 陪伴 App + 其底层记忆系统 |
| 不是商用可随便用的开源 SaaS | source-available（PolyForm Noncommercial 1.0.0，禁止商用），非 OSI open source；部署者可在非商用前提下自托管 |
...
Nemos 在 Substrate 之上**多加 I1（多租户）**——这是 B2C 部署的反向约束。
...
| **basic-memory / khoj** | 个人知识库 | AGPL/GPL，Nemos 不可派生；Nemos 思路差异在多租户 + B2C + 协议化 |
