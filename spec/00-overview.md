...
本文件是 mnemos 协议族的入口。它定义了 mnemos 是什么、不是什么，以及读完接下来 4 份规范前必须先知道的不变量。

---

## 1. mnemos 是什么

**mnemos 是 AI 应用的多租户个人记忆基础设施**。
...
- 它把用户的导出权当作产品伦理底线——任何 mnemos 部署的数据可在 30 分钟内导出成 JSON-LD + Markdown 双轨格式，跨厂商可移植

它由三个协议层组成：
1. **数据 Schema**（见 `10-data-schema.md`）—— mnemos 协议的核心。所有部署、所有 SKU 共享同一份 schema
...
| 不是 RAG 框架 | 不做检索增强生成的中间件，只做记忆存储 + 协议；RAG 框架可在 mnemos 之上构建 |
| 不是 chat history 存储 | 单次对话 log 是 mnemos 的输入，不是它的产出；mnemos 输出的是抽象、去冗、可审计的"关于用户的事实" |
| 不是训练数据收集器 | mnemos 永不向第三方导出用户数据用于训练；用户对自己数据有 burn 权 |
...
mnemos 同一份协议、同一份 schema、同一组 API/MCP/SDK 接口在三种部署形态下都可运行。

| SKU | 谁运营 | 数据加密 | 索引位置 | 价格 | 协议适配关键点 |
|---|---|---|---|---|---|
| **a. 公共云**（默认） | mnemos 社区/项目方 | 服务端加密（rest + transit） | 服务端 | 免费 + 捐赠覆盖 | 全字段服务端可见；所有 API/MCP 与 SDK 工作模式一致 |
...
迁移路径完全由 export schema 保证；任何 mnemos 部署都必须实现一键 export + 一键 import。

---

## 4. 关键不变量（多租户版的 5 条强约束 → 10 条不变量）

以下 10 条是 mnemos 协议的硬规则。任何 SKU、任何 personality preset、任何 AI app、任何 SDK 实现都不可破坏。
...
### I8. 跨厂商可导出
- 所有 mnemos 部署必实现 §10 export schema（§10 见 `10-data-schema.md`）
- Export tar.gz 可被任意 mnemos 部署 import，且 round-trip checksum = 0
...
mnemos 在 Substrate 之上**多加 I1（多租户）**——这是 OSS B2C 部署的反向约束。

---

## 5. 与现有方案的差异

| 项目 | 主要思路 | 与 mnemos 的差异 |
|---|---|---|
| **mem0** | personal memory layer for LLM apps，向量驱动 | mem0 是单租户向量库 + chat history wrapper；mnemos 是多租户 5 层 schema + 三维元数据 + ownership 分类，向量只是一个 facet |
| **Letta (MemGPT)** | agent core memory + recall memory，分级 | Letta 是 agent-runtime 内的记忆；mnemos 是 agent-independent infra，多 agent 共享同一份用户记忆 |
| **OpenMemory / Mem0 OSS** | 个人 OSS 记忆 layer | OpenMemory 单租户 markdown；mnemos 多租户 + E2EE 兼容 + 三 SKU |
| **Cognee** | knowledge graph + cognitive cycles | Cognee 偏 graph；mnemos 偏关系 + 时间 + ownership；可在 mnemos 上叠 graph 层 |
| **MCP (Anthropic)** | tool/resource protocol | MCP 是协议层；mnemos 是 MCP server 实现 + REST 双协议；mnemos 必然适配 MCP 但不仅是 MCP |
| **Memory-Palace** | provenance + immutable | Memory-Palace 思路与 mnemos archival 一致，但单租户；mnemos 借鉴 provenance |
| **basic-memory / khoj** | 个人知识库 | AGPL/GPL，mnemos 不可派生；mnemos 思路差异在多租户 + B2C OSS + 协议化 |
| **Memori / memobase** | profile 抽象 | mnemos personal_semantic 借鉴 facet 设计 |

**mnemos 与所有现有方案的根本差异**：它不是"应用内记忆模块"，它是"用户主导的、跨应用的、可携带的、可导出的、可纠正的记忆 infra"。AI app 是它的客户，用户是它的所有者。
...
- `70-federation.md`：mnemos 节点之间联邦同步协议
...
### Q4. Relational Store 的 ACL 模型在跨 mnemos 部署时的 PKI
- **不确定**：alice 在 mnemos.com（SKU a）+ user 自托管 SKU c，怎么互相签名？需要 PKI 网络 / Web of Trust / fediverse-style identity
...
- **不确定**：MCP resources 设计上是"可订阅资源"；mnemos 是否应把整个 memory 当 resource 暴露 vs 只暴露过滤后的 view
...
- **不确定**：mnemos 想做"协议化"，但其他 OSS 项目已经有自己的 schema；强行对齐会拖慢 mnemos，不对齐会变孤岛
- **Round 2 路径**：v0.1 spec 独立发布；同时发 import adapter（`mem0_to_mnemos.py` / `letta_to_mnemos.py`）做单向迁移；不做双向语义绑定
...
- **Round 2 路径**：v0.1 只保证 mnemos 服务端 + 官方 SDK cache 的回收；第三方 cache 由 agent 自行实现 + audit log 记录"burn 已发起"
