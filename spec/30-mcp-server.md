...
- name: propose_personal_semantic
  description: |
    ...
    User explicitly accepts the proposal -> nemos writes it with source.kind=authoritative.
...
```
nemos://memory/<scope>/<view-name>
```

示例：
- `nemos://memory/scope:work/personal-semantic-summary`
- `nemos://memory/global/recent-narrative-events`
- `nemos://memory/scope:health/forbidden`  ← 返回 403 (capability denied)

### 3.2 v0.1 暴露的 resources

| URI 模板 | 内容 | 订阅频率 |
|---|---|---|
| `nemos://memory/<scope>/personal-semantic-summary` | 该 scope 内 personal_semantic 的简化 view（key+value+confidence） | on change（manifest 允许的范围） |
| `nemos://memory/<scope>/active-period` | 当前 active period 元数据 | on change |
| `nemos://memory/<scope>/recent-episodic` | 最近 N 条 episodic | 5 min poll |
| `nemos://memory/<scope>/procedural-patterns` | 已成形的 procedural patterns | on change |
| `nemos://memory/<scope>/proposals` | 该 scope 下 AI 提议的 personal_semantic（待用户审批） | on change |
| `nemos://meta/capability-self` | 本 agent 的当前 capability | on change |
| `nemos://meta/schema-version` | 服务端 schema version | static |
...
// Client → Server
{
  ...
  "params": {
    "uri": "nemos://memory/scope:work/personal-semantic-summary"
  }
}
...
| 安装 | 每台机器装 nemos-mcp binary | 注册 cloud account | 装 local proxy + cloud account |
...
`nemos-mcp` binary 从 config 读 endpoint，转换 MCP JSON-RPC ↔ REST HTTP。
...
```
AI app ↔ local nemos-mcp (stdio) ↔ local key store + 客户端 SQLite + HNSW
                                  ↘ remote nemos cloud (密文 sync only)
```
...
部署者跑 `nemos-server` + `nemos-mcp` 在同一台机器；MCP server 通过 in-process call 直接调 service handler，不走 HTTP。
