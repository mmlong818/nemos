# mnemos v0.1 — SDK 接口契约 (40-sdk-contract)
...
```
                    ...
                    └──────────┬──────────┘
                               ...
                               ▼
                    ┌─────────────────────┐
                    │   mnemos REST API   │  ← 见 20-rest-api.md
                    └─────────────────────┘
...
```typescript
import { Mnemos, MemoryRecord, Source, Arousal, Surprise } from '@mnemos/sdk';

const client = new Mnemos({
  endpoint: 'https://api.mnemos.org/v1',
  apiKey: process.env.MNEMOS_CAPABILITY_JWT,
  agentPrivateKey: process.env.MNEMOS_AGENT_KEY,    // for ed25519 sign
  sku: 'a',                                          // 'a' | 'b' | 'c'
  e2eeKey: undefined,                                // SKU b 必填
  cache: { l1Max: 100, l2Path: '~/.mnemos/cache.db' }
});
```
...
const mnemos = new Mnemos({
  endpoint: 'https://api.mnemos.org/v1',
  apiKey: process.env.MNEMOS_JWT!,
  agentPrivateKey: process.env.MNEMOS_AGENT_KEY!
});

// session 开始
const session = await mnemos.injectSessionStart({
  appOrigin: 'cursor',
  scopeId: 'scope:work',
  periodId: 'active',
  want: {
...
// hot-path query
const result = await mnemos.query({
  text: '用户的写作偏好',
...
// 写入观察
const written = await mnemos.writeEpisodic({
...
await mnemos.proposePersonalSemantic({
  facet: 'preference',
  key: 'writing_time',
  value: 'early morning',
...
```python
from mnemos import Mnemos, MemoryRecord, Source, Arousal, Surprise

client = Mnemos(
    endpoint="https://api.mnemos.org/v1",
    api_key=os.environ["MNEMOS_CAPABILITY_JWT"],
    agent_private_key_path=os.environ["MNEMOS_AGENT_KEY_PATH"],
    sku="a",
    cache={"l1_max": 100, "l2_path": "~/.mnemos/cache.db"}
)
```
...
client = Mnemos(
    endpoint="https://api.mnemos.org/v1",
    api_key=os.environ["MNEMOS_JWT"],
    agent_private_key_path="~/.mnemos/agent.key"
)
...
| 项 | TS | Python | 说明 |
|---|---|---|---|
| endpoint | `endpoint` | `endpoint` | mnemos server base URL |
...
| Env var | 用途 |
|---|---|
| `MNEMOS_ENDPOINT` | endpoint |
| `MNEMOS_API_KEY` | capability JWT |
| `MNEMOS_AGENT_KEY` | agent ed25519 私钥 PEM |
| `MNEMOS_E2EE_KEY` | E2EE 密钥（hex/base64） |
| `MNEMOS_SKU` | 'a' / 'b' / 'c' |
| `MNEMOS_CACHE_DIR` | L2/L3 SQLite mirror 路径 |
...
```typescript
class MnemosError extends Error {
  code: string;                          // mnemos_error_code
...
class MnemosInvariantError extends MnemosError {}     // 4xx invariant violation
class MnemosCapabilityError extends MnemosError {}    // 403
class MnemosRateLimitError extends MnemosError {
  retryAfterSeconds: number;
}
class MnemosNetworkError extends MnemosError {
  retryable: true;
}
```
...
```typescript
const client = new Mnemos({ ..., offlineMode: { enabled: true, queuePath: '~/.mnemos/queue.db' } });
```
...
- 默认上限 1000 条 / 100MB
- 超出 → 拒绝新写入 + raise `MnemosQueueFullError`
...
- **S8**: SDK 在 schema_version 不匹配时 raise `MnemosSchemaMismatchError`，不静默降级
...
- `@mnemos/sdk` 版本独立于 mnemos schema_version
- SDK SemVer 跟 mnemos schema major：
...
> - 起 mnemos-server reference implementation（Go single-binary，SKU c first）
