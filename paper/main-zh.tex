...
\author{Nemos 开发团队\thanks{工作草稿。所报告的全部数字均来自随附评测框架中冻结的 $n{=}50$ 实验运行。}\\
\texttt{s181@163.com}\\
...
\section{实验设置}
\label{sec:setup}
所有系统的抽取/反思均用 OpenAI \texttt{gpt-4o}，嵌入用 \texttt{text-embedding-3-small}；评判器为 \texttt{gpt-4o}。每个任务报告 $n=\nItems{}$ 条样本。记忆系统一侧在瞬时错误重试全部失败后按\emph{空检索}计分（其全部探针记为 miss，与外部基线路径一致）；评判器调用重试全部失败则将该探针从比率中剔除并单独上报（冻结跑批中此类失败为零）。每个任务恰好消融受测的那一个机制，从而任何差异都可归因于该机制：
\begin{itemize}
