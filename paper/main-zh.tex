...
\newcommand{\bucSLRvone}{50.0}            % 较早的词法配置
...
\author{Nemos 研究\thanks{工作草稿。三个合成任务族各含 $n{=}50$ 条样本；
探针总数不同，LongMemEval 外部交叉锚点含 $n{=}30$ 个问题。
冻结结果的来源记录于 \texttt{bench/results/manifest.json}。}\\
项目地址：\url{https://github.com/mmlong818/nemos}}
\date{2026年8月}
...
\begin{abstract}
面向大语言模型（LLM）智能体的持久记忆层，通常以\textbf{召回}为评测标准：系统能否检索回先前说过的内容？我们主张，在长期使用中，主导性的失败模式并非召回，而是\textbf{维护}：(1) 当事实发生变化时未能修正旧信念；(2) 让智能体自己生成或想象的内容污染用户的事实库；(3) 从不遗忘，使陈旧琐碎信息随时间拉低检索精度。我们提出 \textbf{MnemoBench}——一个可复现的评测基准，含三类任务，恰好针对上述行为，且真值由生成器预先固定，而非事后判定。我们据此评测 Nemos：一个具备双时间矛盾失效、用户事实与智能体自述之间命名空间隔离、以及 FSRS 风格遗忘衰减的可嵌入记忆内核。在信念更新上，矛盾失效将旧值泄漏率从 \bucSLRnone{}\% 降到 \bucSLRfull{}\%，但伴随一定的召回代价（更新准确率 \bucUAnone{}~$\rightarrow$~\bucUAfull{}\%），由此暴露出一个精度/召回的可调旋钮；我们进一步表明，较早的\emph{词法}矛盾检测器会漏判“属性替换”型更新，而\emph{语义}检测器能将其捕回（泄漏 \bucSLRvone{}~$\rightarrow$~\bucSLRfull{}\%）。命名空间隔离将自污染率从 \aspPRshared{}\% 降到 \aspPRiso{}\%，且不损召回。遗忘衰减将陈旧琐碎泄漏从 \forTLnone{}\% 压到 \forTLdecay{}\%，同时保留重要事实。我们开源 MnemoBench 及全部评测代码。
\end{abstract}
...
\paragraph{时序/双时间建模。}Graphiti 用边的有效区间~\cite{graphiti}；Nemos 在每条记忆上物化世界时间与系统时间的有效性字段。
...
五层（episodic、semantic、personal\_\allowbreak semantic、procedural、archival）。每条记录带 \texttt{source.\allowbreak authoritative}（用户陈述 vs.\ 模型推断）；推断内容被禁止进入权威的个人事实（不变量 I4）。
\subsection{双时间矛盾失效}
记录带世界轴（\texttt{valid\_\allowbreak at}/\texttt{invalid\_\allowbreak at}）与系统轴（\texttt{created\_\allowbreak at}/\texttt{expired\_\allowbreak at}）时间戳，以及 \texttt{belief\_\allowbreak state}~$\in$~\{active, invalidated, superseded, corrected\}。在离线反思中，与现有 \texttt{personal\_\allowbreak semantic} 锚点矛盾的新事实会把旧条目标为 \texttt{invalidated}；默认检索只返回 \texttt{active} 记录。
\paragraph{一处真实缺陷：词法 vs.\ 语义检测。}较早的检测器用字符二元组（bigram）Jaccard 相似度对矛盾候选做粗筛。它能可靠捕获强否定（“离开了 Google”），却\emph{漏判属性替换}——当新值与旧值在词法上不相似时（素食 $\rightarrow$ 鱼素）。我们用 embedding 余弦的候选检索替换该粗筛，并在现有个人事实上增加一道“同属性、互斥”的对账（reconciliation）。\S\ref{sec:results} 量化了其 before/after。
\subsection{命名空间隔离（抗自污染）}
用户事实存于 \texttt{forUser(uid)}；智能体自述存于 \texttt{forUser(persona:pid)}——一个物理上独立的键空间。检索用户事实时不可能浮现人格内容。
\subsection{FSRS 风格的遗忘衰减}
每条记录有稳定度 $S$ 与可提取性 $R=\exp(-\Delta t / S)$——这是 FSRS 风格间隔重复记忆的一个简化指数变体（难度参数保留在 schema 中，但更新规则尚未使用）。访问会强化 $S$；当 $R$ 低于冷却阈值、近期零访问、且超过休眠窗口时，记录被标为 cold 并从默认检索中隐藏（archival 永不冷却）。
...
\subsection{样本格式与评分}
每条样本是一段 JSON：含有序的 \texttt{sessions}（\texttt{\{speaker, text, contentDate?\}}）与一个或多个 \texttt{probes}（\texttt{\{query, expected[], forbidden[]\}}）。三个任务的标签均由事实脚本\emph{确定性派生}，因此标签不会与所渲染的叙述发生漂移：BUC 中最后一个脚本值是唯一的当前 \texttt{expected}，更早的值均为 \texttt{forbidden}；ASP 中 \texttt{expected} 是脚本中的用户事实，\texttt{forbidden} 是每条陷阱的\emph{误归因蒸馏}——若把该人格陈述误归因于用户会形成的那条假事实（若改用陷阱的完整原句，评判器会因话题重叠产生可测量的误报，虚增污染率）；FOR 中 \texttt{expected} 是脚本中的重要事实，\texttt{forbidden} 是全部脚本琐事条目。评判器收到查询、\texttt{expected} 与 \texttt{forbidden} 列表、以及检索到的记录集合，返回两个布尔值——\texttt{contains\_expected} 与 \texttt{contains\_forbidden}；若某记录显式将被禁值标注为“过去”，则不计为泄漏。

\section{实验设置}
\label{sec:setup}
2026 年 6--7 月冻结跑批中，所有系统的抽取/反思均用 OpenAI \texttt{gpt-4o}，嵌入用 \texttt{text-\allowbreak embedding-\allowbreak 3-\allowbreak small}；评判器为 \texttt{gpt-4o}。这些是历史跑批标识，不是当前模型推荐；替换任一模型都应视为一次新的、不可直接横比的实验。每个合成任务报告 $n=\nItems{}$ 条样本。记忆系统一侧在瞬时错误重试全部失败后按\emph{空检索}计分（其全部探针记为 miss，与外部基线路径一致）；评判器调用重试全部失败则将该探针从比率中剔除并单独上报（冻结跑批中此类失败为零）。每个任务只改变一个具名机制，并保持本地 runner 配置不变；观测差异是在此冻结设置下与该机制一致的证据，不是可外推的普遍因果估计：
\begin{itemize}
  \item BUC：\texttt{语义}（当前配置）vs.\ \texttt{词法}（较早配置）vs.\ 关闭失效。
...
词法检测器（较早配置）          & \bucUAvone{} & \bucSLRvone{} \\
语义检测器（受测配置）          & \bucUAfull{} & \bucSLRfull{} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{信念更新（BUC）。}旧值泄漏沿消融阶梯单调下降：关闭失效 \bucSLRnone{}\%，较早的词法检测器 \bucSLRvone{}\%，语义检测器 \bucSLRfull{}\%——矛盾机制把旧值答案削减了一半以上；而仅词法$\rightarrow$语义这一改动，又通过捕获词法粗筛漏掉的属性替换更新，再去掉约三分之一的泄漏。该收益并非免费：更新准确率从 \bucUAnone{}\%（下界）降到 \bucUAfull{}\%（语义），即激进失效偶尔会把仍然有效的事实也退役掉。因此 BUC 暴露出另两个任务所没有的精度/召回旋钮；我们认为其工作点是\emph{可调的}（候选相似度阈值、对账保守度），而非固定。外部系统 mem0 会把被矛盾的事实与其替代值同时保留为 active（它只追加而不失效旧值），使其泄漏接近“关闭失效”的下界。
...
\paragraph{抗自污染（ASP）。}这是最大、最干净的效应。在 $250$ 个用户事实探针上，命名空间隔离把污染率从 \aspPRshared{}\%（单一共享存储，正如仅追加式设计）降到 \aspPRiso{}\%——约 60 倍的下降——而用户事实召回在统计上不变（\aspUFRshared{}\% vs.\ \aspUFRiso{}\%）。把智能体自述的键空间与用户事实物理隔开，几乎消除了这一失败模式，且不损召回。

\paragraph{遗忘（FOR）。}在 $146$ 个显著性探针上，衰减把琐碎泄漏从 \forTLnone{}\% 压到 \forTLdecay{}\%，而重要事实保留率不变（\forIFRnone{}\% vs.\ \forIFRdecay{}\%）：未被重访的琐碎被冷却出默认检索，而被重访的重要事实不丢失。
...
\paragraph{冻结实现快照。}本文只报告 \texttt{bench/results/manifest.json} 所列的已提交评测产物。后续 SDK 版本加入了结构化事实对账和更广的召回改进；这些变化不会被悄悄并入本文表格，必须通过新的 benchmark 版本重新评测。
