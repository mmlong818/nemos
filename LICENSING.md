...
本仓库**自有代码统一**按 PolyForm Noncommercial 1.0.0 授权，见 `LICENSE`。这份文件是授权范围的
权威说明：它讲清该许可证覆盖到哪里、哪些部分不由它决定，以及随包分发的第三方代码各自适用什么条款。
...
| **小丑鱼应用** | `sdk/typescript/examples/companion/` | PolyForm Noncommercial 1.0.0（见 `LICENSE`） | 2026-09-09 起与仓库其余代码统一；商业用途需另行取得授权 |
| **示例与基准** | `sdk/typescript/examples/`（companion 以外）、`bench/` | 同 SDK | 用于说明与复现 |

## 为什么统一到一个许可证

2026-09-09 起小丑鱼应用与仓库其余代码使用同一个许可证。

此前应用被单独划为「保留全部权利」，写的理由是「若应用沿用该许可证，销售行为将与自身授权条款
冲突」。**那条理由是错的**，而且与下一节自己的结论互相矛盾：许可证约束的是被许可方，不约束
著作权人。权利人把代码以 PolyForm Noncommercial 对外发布之后，依然可以销售同一份代码，也可以
另售商业授权。所以拆分并没有保护到任何实际利益，只是让授权边界更难解释。

统一之后真正改变的是**别人获得的权利**：任何人现在都可以非商业地使用、修改和分发小丑鱼，
而且该授权对已发布的版本不可撤回。这是做这个决定时要接受的代价。PolyForm 对「非商业」的
定义偏严，公司内部使用不属于非商业用途，仍需另行取得授权。
...
根 `LICENSE` 是 PolyForm Noncommercial 的许可证正文，**不是**最终用户许可协议，
也不构成销售条款。应用目录下原来那份权利保留声明已随本次统一删除。
